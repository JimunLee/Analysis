...
void DrawBW()
{
  ...
  TH1D* hEfficiency[0];
  hEfficiency[0] = new TH1D("Closure", "Closure", nPtBins,xBins);
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  fkstar[i][j]->SetParameter(2, kstarwidth);
	  ...
	  fFit[i][j]->SetParLimits(4, 0.930, 0.98);
	  ...
	  double BgY = (1/binW)*totbg[i][j]->Integral(0.85,0.95);
	  ...
	  // Efficiency
	  //=============================
	  double hist_efficiency = 0;
   
	  double lowBin  = hUSSMC_True[i][j]->FindBin(0.85);
	  double highBin = hUSSMC_True[i][j]->FindBin(0.95);

	  double Rec_yield = (1/binW)*bw[i][j]->Integral(0.85, 0.95);
	  double Gen_yield = hUSSMC_True[i][j]->Integral(lowBin, highBin);
	  hist_efficiency = Rec_yield/Gen_yield;
	  
	  hEfficiency[0]->SetBinContent(i+1, hist_efficiency);
	  
	  cout <<"Rec Yield: " << Rec_yield <<endl;
	  cout <<"Gen Yield: " << Gen_yield <<endl;
	  cout <<"Efficiency: " << hist_efficiency <<endl;
	  cout << "//==========================================================//" << endl;
	} //for j
    }//for i
  ...
  hEfficiency[0]->Draw();
  c->SaveAs("../kstar-in-oo/Plot/"+localpath+"hEfficiency.png", "RECREATE");
