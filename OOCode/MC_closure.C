...
void DrawBW()
{
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  TLatex* txtInfogenB = DrawTLatex(0.191, 0.874, "O-O yield", 0.04, 62, 1);
	  TLatex* txtInfogenC = DrawTLatex(0.191, 0.820, "o-o #sqrt{s_{NN}} = 5.36 TeV", 0.04, 42, 1);
	  ...
	  for (int b = 1; b <= hYield[0]->GetNbinsX(); ++b) {
	    cout << "bin " << b
		 ...
		 << " True= " << hTrueYield[0]->GetBinContent(b) << endl;
	  }
