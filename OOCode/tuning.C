...
void DrawBW()
{
  ...
  for (int i=4; i<5; i++)
      ...
      for (int j=3; j<4; j++)
	{
	  ...
	  for (int bin = hUSSFit[i][j]->FindBin(1.06); bin <= hUSSFit[i][j]->FindBin(1.20); bin++)
	    hUSSFit[i][j]->SetBinError(bin, 0);

	  //=================================================================	  
	  fFit[i][j]->SetParLimits(0, 0, 1700);
	  fFit[i][j]->SetParLimits(1, 0.75, 0.79);
	  fFit[i][j]->SetParLimits(2, 0.070, 0.120);
	  
	   fFit[i][j]->SetParLimits(3, 0, 1000);
	   fFit[i][j]->SetParLimits(4, 0.928, 0.980);
	  ...
	  /* /\* TCanvas* cent = new TCanvas(Form("Cent_%d_%d", i,j), "", 940, 800); *\/ */
	  /* /\* hCent[i][j]->Draw(); *\/ */
	  ...
	  /* //  double totEvents = 0.0; */
	  
	  /* if (j==0) */
	  /*   nEvents = hCent[i][j]->Integral(1, hCent[i][j]->FindBin(10)); //from the number of bin 1 to 10 */
	  /* else if (j!=0) */
  ...
  /* double yMin = hYield[4]->GetMinimum(); */
