...
void DrawBW()
{
  ...
  for (Int_t i=0; i<nPtBins; i++)
      ...
      for (Int_t j=0; j<nCentBins; j++)
	{
	  ...
	  TLatex* txtInfoC = DrawTLatex(0.522, 0.264, "o-o #sqrt{s_{NN}} = 5.38 TeV", 0.04, 42, 1);
  ...
  TH1D* hclosure[0];
  hclosure[0] = new TH1D("", "", nPtBins,xBins);
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  TLatex* txtInfoCC = DrawTLatex(0.173, 0.820, "o-o #sqrt{s_{NN}} = 5.38 TeV", 0.04, 42, 1);
	  ...
	  double HistY = 0;
	  double HistYerr = 0;
	  double lowM = hUSSFit[i][j]->FindBin(0.70);
	  ...
	  double BgY = (1/binW)*totbg[i][j]->Integral(0.70,1.10);
	  ...
	  Double_t Bg_err = (1/binW)*totbg[i][j]->IntegralError(0.70, 1.10, totbg[i][j]->GetParameters(), covGG_err.GetMatrixArray());
	  ...
	  // Closure test
	  //=============================
	  double lowBin  = hUSSMC_True[i][j]->FindBin(0.70);
	  double highBin = hUSSMC_True[i][j]->FindBin(1.10);

	  /* double Rec_yield = (1/binW)*bw[i][j]->Integral(0.85, 0.95); */
	  /* double Gen_yield = hUSSMC_True[i][j]->Integral(lowBin, highBin); */
	  /* double h_closure = Rec_yield/Gen_yield; */

	  //TH1D* hclosure[i][j] = (TH1D*)bw[i][j]->Clone(Form("h_closure_%d_%d", i, j));
	  //hclosure[i][j]->Divide(hUSSMC_True[i][j]);
	
	  //hclosure[i][j]->Draw();
	  
	  
	  //hclosure[j]->SetBinContent(i+1, h_closure);
	  
	  /* cout <<"Rec Yield: " << Rec_yield <<endl; */
	  /* cout <<"Gen Yield: " << Gen_yield <<endl; */
	  //cout <<"Closure: " << hclosure <<endl;
	  cout << "//==========================================================//" << endl;
	} //for j
    }//for i
  ...
  HistoAxisTitles(hdummy, "", "#frac{d^{2}N_{K^{*0}}}{d#it{p}_{T}d#eta}",
  ...
  TLatex* txtInfoBBB = DrawTLatex(0.59, 0.789, "o-o, #sqrt{s_{NN}} = 5.38 TeV", 0.05, 42, 1);
  ...
  for(int j=0; j<3; j++){	    
    hclosure[j]->SetLineWidth(1);
    hclosure[j]->SetLineColor(red0);
    hclosure[j]->SetMarkerStyle(20); //circle
    hclosure[j]->SetMarkerSize(1);
    hclosure[j]->SetMarkerColor(red0);
    hclosure[j]->Draw("SAME");
  }
