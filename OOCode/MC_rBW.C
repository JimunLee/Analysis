...
double rBW(double *x, double *par) // parameter 3
{
  double xx = x[0];
  double A = par[0];
  double M0 = par[1];
  ...
  double Gamma = pow((xx*xx-4*Mpi*MK0s)/(M0*M0-4*Mpi*MK0s), 1.5);
  Gamma *= Gamma0 * M0 /xx;

  double val = (A*xx*M0*Gamma) / ((M0*M0 -xx*xx)*(M0*M0-xx*xx) + M0*M0*Gamma*Gamma);
  return val;
}
