...
void DrawBW()
{
  ...
  // Data file inputs 
  TString mainfile = "../kstar-in-oo/Result_rootfile/apass2_DATA_small_ACDCA.root";
  TFile* data = TFile::Open(mainfile);
  if(!data)   return;

  TString USSKPiName("kstar-in-o-o/hUSS_KPi;1");
  TString USSPiKName("kstar-in-o-o/hUSS_PiK;1");
  TString LSSKPiName("kstar-in-o-o/hUSS_KPi_Mix;1");
  TString LSSPiKName("kstar-in-o-o/hUSS_PiK_Mix;1");
  ...
  for (Int_t i=0; i<nPtBins; i++)
    {
      ...
      //      TH1D* centtest = (TH1D*)hUSS_KPi_MC->Projection(0);
      ...
      for (Int_t j=0; j<nCentBins; j++)
	{
	  ...
	  (hLSS_KPi_MC->GetAxis(0))->SetRangeUser(40, 80);
	  (hLSS_PiK_MC->GetAxis(0))->SetRangeUser(40, 80);
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  fBg2[i][j] = new TF1(Form("kstar_GausBg_%i_cent_%i",i,j), "gaus(0)+pol2(3)", BgMinRange, BgMaxRange);
	  fFit[i][j] = new TF1(Form("kstar_fullFit_%i_cent_%i",i,j), "gaus(0)+ pol2(3) + [6]*BreitWignerRelativistic(x, [7], [8])", BgMinRange, BgMaxRange);
	  ...
	  fFit[i][j]->SetParLimits(1, 0.745, 0.81);
	  fFit[i][j]->SetParLimits(2, 0.0143, 0.09);

	  //fFit[i][j]->SetParLimits(3, 0, 40000);
	  /* fFit[i][j]->SetParLimits(4, 0.960, 0.999); */
	  /* fFit[i][j]->SetParLimits(5, 0.11, 0.187); */
	  
	  fFit[i][j]->SetParameter(3, fBg2[i][j]->GetParameter(3));
	  fFit[i][j]->SetParameter(4, fBg2[i][j]->GetParameter(4));
	  fFit[i][j]->SetParameter(5, fBg2[i][j]->GetParameter(5));
	  
	  fFit[i][j]->SetParameter(6, fkstar[i][j]->GetParameter(0)); //DO NOT FIX      
	  fFit[i][j]->FixParameter(7, fkstar[i][j]->GetParameter(1));
	  fFit[i][j]->FixParameter(8, fkstar[i][j]->GetParameter(2));
	  ...
	  totbg[i][j] = new TF1(Form("totbg_%i_cent_%i",i,j), "gaus(0) + pol2(3)", BgMinRange, BgMaxRange);
	  ...
	  pol2bg[i][j]->FixParameter(0, fFit[i][j]->GetParameter(3));
	  pol2bg[i][j]->FixParameter(1, fFit[i][j]->GetParameter(4));
	  pol2bg[i][j]->FixParameter(2, fFit[i][j]->GetParameter(5));
   
	  bw[i][j]->FixParameter(0, fFit[i][j]->GetParameter(6));
	  bw[i][j]->FixParameter(1, fFit[i][j]->GetParameter(7));
	  bw[i][j]->FixParameter(2, fFit[i][j]->GetParameter(8));
