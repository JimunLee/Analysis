  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  if(i==0)
	    ...
	    fFit[i][j]->SetParameter(0, fFit[i-1][j]->GetParameter(0));
	  
	  // fFit[i][j]->SetParLimits(1, 0.725, 0.814);
	  fFit[i][j]->SetParLimits(1, 0.735, 0.785);
	  fFit[i][j]->SetParLimits(2, 0.025, 0.120);
	  

	  if(i==0)
	    fFit[i][j]->SetParLimits(3, 0, 30000);
	  else
	    fFit[i][j]->SetParameter(3, fFit[i-1][j]->GetParameter(3));
	  //fFit[i][j]->SetParLimits(3, 0, 7000);
	  
	  fFit[i][j]->SetParLimits(4, 0.944, 0.999);
