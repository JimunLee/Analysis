  ...
  for (int k=0; k<nPtBins; k++)
    {
      ...
      ff0[k] = new TF1(Form("f0_%i",k), "[0]*BreitWignerRelativistic(x, [1], [2])", BgMinRange, BgMaxRange);
      fBg[k] = new TF1(Form("exp_%i",k), "exp([0] + [1]*x + [2]*x*x)*pol2(3)+[6]*BreitWignerRelativistic(x, [7], [8])", BgMinRange, BgMaxRange);
      fFit[k] = new TF1(Form("kstar_fullFit_%i",k), "exp([0]+[1]*x+[2]*x*x)+[3]*BreitWignerRelativistic(x, [4], [5])+[9]*BreitWignerRelativistic(x, [10], [11])", BgMinRange, BgMaxRange);
      ...
      ff0[k]->SetParameter(1, 0.975); 
      ff0[k]->SetParLimits(2, 0.080, 0.152);
	...
	CloneHist->SetBinError(b, hUSSBg[k]->GetBinError(b));
      ...
      fBg[k]->SetParameter(7, ff0[k]->GetParameter(1));
      fBg[k]->SetParameter(8, ff0[k]->GetParameter(2));
      
      CloneHist->Fit(fBg[k], "SR0", "", BgMinRange, BgMaxRange);
      CloneHist->Draw();
      fBg[k]->Draw("SAME");
      continue;
      //=================================================================
      TH1D* CloneHist2 = new TH1D("test", "test", nBins/5, imlow, imtop);
      for (int b=0; b<nBins; b++){
	if(b<24 || b>35)
	  continue;

	CloneHist2->SetBinContent(b, hUSSkstarSig[k]->GetBinContent(b));
	CloneHist2->SetBinError(b, hUSSkstarSig[k]->GetBinError(b));
      }
      CloneHist2->Fit(fkstar[k], "SR0", "", 0.86, 0.94);
      /* CloneHist2->Draw(); */
      /* fkstar[k]->Draw("SAME"); */
      /* continue; */
      ...
      expbg[k] = new TF1(Form("expbg_%i",k), "exp([0] + [1]*x + [2]*x*x)+[3]*BreitWignerRelativistic(x, [4], [5])", BgMinRange, BgMaxRange);
      ...
      expbg[k]->FixParameter(5, fFit[k]->GetParameter(5));
      /* expbg[k]->FixParameter(6, fFit[k]->GetParameter(6)); */

      
      bw[k]->FixParameter(0, fFit[k]->GetParameter(9));
      bw[k]->FixParameter(1, fFit[k]->GetParameter(10));
      bw[k]->FixParameter(2, fFit[k]->GetParameter(11));
      ...
      legendd->AddEntry(expbg[k], "expbg", "l");
      legendd->Draw("SAME");
