...
void DrawBW()
{
  ...
  for (Int_t i=0; i<nPtBins; i++)
      ...
      for (Int_t j=0; j<nCentBins; j++)
	{
	  ...
	  TH1D* hShade2 = (TH1D*)hLSSPiK[i][j]->Clone(Form("hShade2_%d_%d", i, j));
	  ...
	  hUSS_PiK_SUB2[i][j] = (TH1D*)hUSSPiK[i][j]->Clone(Form("hUSSMix_Minv_%1.1f_%1.1f_cent_%1.1f_%1.1f", ptlow, pthigh, 0.0, 100.0));
	  hUSS_PiK_Mix_SUB[i][j] = (TH1D*)hUSSPiKMix[i][j]->Clone(Form("hUSSMixBg_Minv_%1.1f_%1.1f_cent_%1.1f_%1.1f", ptlow, pthigh, 0.0, 100.0));
	  hUSS_PiK_SUB2[i][j]->Add(hUSS_PiK_Mix_SUB[i][j], -1);
	}//for2
    }//for1
