  ...
  for (int k=0; k<1; k++)
    {
      ...
      // fBg2[k] = new TF1(Form("kstar_GausBg_%i",k), "gaus(0)+pol3(3)+([7]*BreitWignerRelativistic(x, [8], [9]))", BgMinRange, BgMaxRange);
      fFit[k] = new TF1(Form("kstar_fullFit_%i",k), "gaus(0)+pol3(3)+[7]*BreitWignerRelativistic(x, [8], [9])", BgMinRange, BgMaxRange);

      
      fBg2[k] = new TF1(Form("kstar_GausBg_%i",k), "gaus(0)+pol3(3)", BgMinRange, BgMaxRange);
	...
	CloneHist->SetBinError(b,hUSSBg[k]->GetBinError(b));
      ...
      fBg2[k]->FixParameter(1, 0.732);
      fBg2[k]->FixParameter(2, 0.062);
      
      fBg2[k]->SetParLimits(3, 0, 1000000);
      fBg2[k]->SetParLimits(4, -1500000,500000);
	...
	CloneHist2->SetBinError(b,hUSSBg[k]->GetBinError(b));
      ...
      fFit[k]->FixParameter(6, fBg2[k]->GetParameter(6));
      
      fFit[k]->SetParameter(7, fkstar[k]->GetParameter(0)); //DO NOT FIX      
      fFit[k]->FixParameter(8, fkstar[k]->GetParameter(1));
      fFit[k]->FixParameter(9, fkstar[k]->GetParameter(2));
      ...
      TF1* pol3bg = new TF1("pol3bg", "pol3(0)", BgMinRange, BgMaxRange);
      ...
      pol3bg->FixParameter(0, fFit[k]->GetParameter(3));
      pol3bg->FixParameter(1, fFit[k]->GetParameter(4));
      pol3bg->FixParameter(2, fFit[k]->GetParameter(5));
      pol3bg->FixParameter(3, fFit[k]->GetParameter(6));

      bw->FixParameter(0, fFit[k]->GetParameter(7));
      bw->FixParameter(1, fFit[k]->GetParameter(8));
      bw->FixParameter(2, fFit[k]->GetParameter(9));

      gausbg->SetLineColor(6); //magenta
      gausbg->Draw("SAME");
      
      pol3bg->SetLineColor(4); //blue
      pol3bg->Draw("SAME");
