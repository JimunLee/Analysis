...
void DrawBW()
{
  ...
  // Data file inputs 
  TString mainfile = "../kstar-in-oo/Result_rootfile/OODataAnalysisResults.root";
  TFile* data = TFile::Open(mainfile);
  if(!data)   return;

  TString USSName("kstar-in-o-o/hUSS;1");
  TString MixedUSSName("kstar-in-o-o/hUSS_Mix;1");
  ...
  THnSparseT<TArrayF> *hUSS_Data = (THnSparseT<TArrayF>*)data->Get(USSName);
  THnSparseT<TArrayF> *hUSS_Mix_Data = (THnSparseT<TArrayF>*)data->Get(MixedUSSName);

  TH1D* hUSS [nPtBins];
  TH1D* hUSS_Mixed [nPtBins];
  TH1D* hUSS_Mixed_SUB [nPtBins];
  TH1D* hUSSData_SUB [nPtBins];
  ...
  for (Int_t i=0; i<nPtBins; i++)
    {
      hUSSData_SUB[i] = new TH1D(Form("hUSSData_SUB_%i", i), Form("hUSSData_SUB_%i", i), nBins, imlow, imtop);
      ...
      ptlow = xBins[i];
      pthigh = xBins[i+1];
      (hUSS_Data->GetAxis(1))->SetRangeUser(ptlow, pthigh);
      (hUSS_Mix_Data->GetAxis(1))->SetRangeUser(ptlow, pthigh);

      hUSS[i] = (TH1D*)hUSS_Data->Projection(2);
      hUSS_Mixed[i] = (TH1D*)hUSS_Mix_Data->Projection(2);
  ...
  for (Int_t j=0; j<nPtBins; j++)
	...
	/ hUSS[j]->Integral(hUSS[j]->FindBin(Nlow), hUSS[j]->FindBin(Nhigh));
      ...
      plots->SaveAs(Form("../kstar-in-oo/Plot/"+localpath+"/Data_projection_%1.1f_%1.1f.png", ptlow, pthigh),"RECREATE");
    
      // ==========================
      // Drawing Substraction
      hUSSData_SUB[j] = (TH1D*)hUSS[j]->Clone(Form("hUSS_Minv_%1.1f_%1.1f", ptlow, pthigh));
      hUSS_Mixed_SUB[j] = (TH1D*)hUSS_Mixed[j]->Clone(Form("hUSS_Mixed_Minv_%1.1f_%1.1f", ptlow, pthigh));
      
      TCanvas* substraction = new TCanvas(Form("Data_substraction_%d", j), "", 550, 440);
      ...
      hUSSData_SUB[j]->Add(hUSS_Mixed_SUB[j], -1);
      HistoAxisTitles(hUSSData_SUB[j], "#it{M}_{inv}^{K^{+}#pi^{-}}[GeV/#it{c}^{2}]", "Counts");
      hUSSData_SUB[j]->SetTitle(Form(""));
      ...
      substraction->SaveAs(Form("../kstar-in-oo/Plot/"+localpath+"/DataRec_reduced_projection_%1.1f_%1.1f.png", ptlow, pthigh)); 
    }//for2
  ...
  double BgMinRange = 0.62;
  double BgMaxRange = 1.15;

  for (int k=0; k<3; k++)
    {
      hUSSkstarSig[k]->Add(hUSSData_SUB[k]);
      hUSSBg[k]->Add(hUSSData_SUB[k]);
      hUSSFit[k]->Add(hUSSData_SUB[k]);
	...
	CloneHist->SetBinError(b,hUSSBg[k]->GetBinError(b)); 
      ...
      fBg2[k]->SetParameter(0, 512212);
      fBg2[k]->SetParLimits(0, 0, 600000);
      fBg2[k]->FixParameter(1, 0.750);
      fBg2[k]->FixParameter(2, 0.066);

      // keep
      //fBg2[k]->FixParameter(2, 0.066);
      
      fBg2[k]->FixParameter(4, 0.970);
      fBg2[k]->FixParameter(5, 0.14);
	...
	CloneHist2->SetBinError(b,hUSSBg[k]->GetBinError(b));
      ...
      fFit[k]->FixParameter(4, fBg2[k]->GetParameter(4));
      fFit[k]->FixParameter(5, fBg2[k]->GetParameter(5));
      ...
      /* legendd->AddEntry(hUSSFit[k],"Data", "lep"); */
      /* legendd->AddEntry(fBg_POST[k], "Bkg. DataFit", "l"); */
      /* legendd->AddEntry(fkstar_POST[k], "BreitWignerRelativistic. DataFit", "l"); */
