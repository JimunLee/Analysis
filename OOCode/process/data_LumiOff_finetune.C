...
void DrawBW()
{
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  fkstar[i][j]->FixParameter(2, 0.0524);
	  
	  //=================================================================
	  fFit[i][j]->SetParLimits(0, 0, 20000);
	  fFit[i][j]->SetParLimits(1, 0.725, 0.810);
	  fFit[i][j]->SetParLimits(2, 0.020, 0.150);

	  if(i==4){
	    fFit[i][j]->SetParLimits(3, 0, 30000);  
	    fFit[i][j]->SetParLimits(4, 0.920, 0.980);
	    fFit[i][j]->SetParLimits(5, 0.055, 0.180);
	  }
	  else{
	    fFit[i][j]->SetParLimits(3, 0, 30000);  
	    fFit[i][j]->SetParLimits(4, 0.950, 0.995);
	    fFit[i][j]->SetParLimits(5, 0.015, 0.120);
	  }
