...
void DrawBW()
{
  ...
  for (Int_t i=0; i<nPtBins; i++)
      ...
      for (Int_t j=0; j<nCentBins; j++)
	{
	  ...
	  hCent[j] = (TH1D*)hUSS_KPi_MC->Projection(0);
	  hCent[j]->SetName(Form("hUSSMC_Cent_%i_%i", i,j));
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  if (j==0){
	    nEvents = hCent[j]->Integral(1, hCent[j]->FindBin(10));
	  }
	  else if (j!=0){
	  ...
	  /* TMatrixDSym covGG(fFit[i][j]->GetNpar()); */
	  /* TMatrixDSym covGG_err(fFit[i][j]->GetNpar()); */
	  /* covTot = ptr->GetCovarianceMatrix(); */
	  /* covGG = covTot.GetSub(9,11,9,11); */
	  /* covGG_err = covTot.GetSub(0,8,0,8);// these have to be totbg parameters */
	  TMatrixDSym covTot = ptr->GetCovarianceMatrix();
	  TMatrixDSym covGG = covTot.GetSub(9,11,9,11);
	  TMatrixDSym covGG_err = covTot.GetSub(0,8,0,8);
	
	  Double_t Bg_err = (1/binW)*(fFit[i][j]->IntegralError(BgMinRange, BgMaxRange, totbg[i][j]->GetParameters(), covGG_err.GetMatrixArray()));
  ...
  cLegend->AddEntry(hYield[0], "0-10% Centrality", "lpf");
  cLegend->AddEntry(hYield[1], "10-30% Centrality", "lpf");
  cLegend->AddEntry(hYield[2], "30-50% Centrality", "lpf");
  cLegend->AddEntry(hYield[3], "50-70% Centrality", "lpf");
  cLegend->AddEntry(hYield[4], "70-100% Centrality", "lpf");
  cLegend->Draw("SAME");
