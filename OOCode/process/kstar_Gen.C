...
void DrawBW()
{
  ...
  for (Int_t j=0; j<nPtBins; j++)
	...
	/ hUSS[j]->Integral(hUSS[j]->FindBin(Nlow), hUSS[j]->FindBin(Nhigh));
      ...
      double delta =1.0;
      hUSS_Mixed[j]->Scale(1.0/Scale*0.99);
  ...
  for (int k=0; k<nPtBins; k++)
    {
      ...
      /* fBg2[k]->SetParLimits(0, 0, 5000); */

      /* fBg2[k]->SetParLimits(1, 0.705, 0.780); */
      /* fBg2[k]->SetParLimits(2, 0.0463, 0.126); */

      /* fBg2[k]->SetParLimits(3, 0, 4000); */
      /* fBg2[k]->SetParLimits(4, 0.968, 1.020); */
      /* fBg2[k]->SetParLimits(5, 0.078, 0.207); */
      ...
      /* CloneHist->Fit(fBg2[k], "SR0", "", 0.60, 1.20); */
      /* CloneHist->Draw(); */
      ...
      /* gausbg->FixParameter(0, fBg2[k]->GetParameter(0)); */
      /* gausbg->FixParameter(1, fBg2[k]->GetParameter(1)); */
      /* gausbg->FixParameter(2, fBg2[k]->GetParameter(2)); */

      /* f0bg->FixParameter(0, fBg2[k]->GetParameter(3)); */
      /* f0bg->FixParameter(1, fBg2[k]->GetParameter(4)); */
      /* f0bg->FixParameter(2, fBg2[k]->GetParameter(5)); */

      /* pol2bg->FixParameter(0, fBg2[k]->GetParameter(6)); */
      /* pol2bg->FixParameter(1, fBg2[k]->GetParameter(7)); */
      /* pol2bg->FixParameter(2, fBg2[k]->GetParameter(8)); */
	...
	CloneHist2->SetBinError(b,hUSSBg[k]->GetBinError(b));
      ...
      if(k==0 || k==1){
	fFit[k]->SetParLimits(0, 0, 240000);
      }
      else if(k==2){
	fFit[k]->SetParLimits(0, 0, 4500);
      }
      
      fFit[k]->SetParLimits(1, 0.695, 0.780);
      fFit[k]->SetParLimits(2, 0.0463, 0.126);

      fFit[k]->SetParLimits(3, 0, 5000);
