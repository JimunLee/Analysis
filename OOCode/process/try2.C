...
void DrawBW()
{
  ...
  for (Int_t j=0; j<nPtBins; j++)
	...
	hUSS_Mixed[j]->Scale(1.0/Scale);
      ...
      hUSSMC_True[j]->Rebin(5);
      
      hUSS[j]->Scale(0.2);
      hUSS_Mixed[j]->Scale(0.2);
      hUSSMC_True[j]->Scale(0.2);
  ...
  for (int k=0; k<nPtBins; k++)
    {
      ...
      // Function
      // ==============
      fkstar[k] = new TF1(Form("kstar_%i",k), "[0]*BreitWignerRelativistic(x, [1], [2])", 0.84, 0.93);
      ...
      //fFit[k]->SetParLimits(3, 0, 90000);
      fFit[k]->FixParameter(4, 0.979);
      fFit[k]->FixParameter(5, 0.137);
      ...
      pol2bg[k]->FixParameter(0, fFit[k]->GetParameter(6));
      pol2bg[k]->FixParameter(1, fFit[k]->GetParameter(7));
      pol2bg[k]->FixParameter(2, fFit[k]->GetParameter(8));
