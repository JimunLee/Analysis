...
void DrawBW()
{
  ...
  TString mainfile = "../kstar-in-oo/Result_rootfile/newAnalysisResults.root";
  ...
  for (Int_t i=0; i<nPtBins; i++)
    {
      ...
      hUSSBg2[i] = new TH1D(Form("hUSSBg2_%i", i), Form("hUSSBg2_%i", i), nBins, imlow, imtop);
      hUSSBg3[i] = new TH1D(Form("hUSSBg3_%i", i), Form("hUSSBg3_%i", i), nBins, imlow, imtop);
      ...
      CloneHist[i] = new TH1D(Form("CloneHist_%i", i), Form("CloneHist_%i", i), nBins/5, imlow, imtop);
  ...
  for (int k=0; k<nPtBins; k++)
    {
      ...
      fBg[k] = new TF1(Form("exp_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x + [4]*x*x*x*x)", BgMinRange, BgMaxRange);
      fFit[k] = new TF1(Form("kstar_fullFit_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x + [4]*x*x*x*x) + [5]*BreitWignerRelativistic(x, [6], [7])", BgMinRange, BgMaxRange);
	...
	if(b>25 && b<33)
	  continue;
	CloneHist[k]->SetBinContent(b, hUSSBg[k]->GetBinContent(b));
	CloneHist[k]->SetBinError(b, hUSSBg[k]->GetBinError(b));
      }
      CloneHist[k]->Fit(fBg[k], "SR0", "", BgMinRange, BgMaxRange);
	...
	CloneHist3->SetBinContent(b, hUSSkstarSig[k]->GetBinContent(b));
	CloneHist3->SetBinError(b, hUSSkstarSig[k]->GetBinError(b));
      }
      CloneHist3->Fit(fkstar[k], "SR0", "", 0.84, 0.94);
      ...
      fFit[k]->SetParameter(5, fkstar[k]->GetParameter(0));
      fFit[k]->FixParameter(6, fkstar[k]->GetParameter(1));
      fFit[k]->FixParameter(7, fkstar[k]->GetParameter(2));
      ...
      expbg[k] = new TF1(Form("expbg_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x + [4]*x*x*x*x)", BgMinRange, BgMaxRange);
      bw[k] = new TF1(Form("bw_%i",k), "[0]*BreitWignerRelativistic(x, [1], [2])", BgMinRange, BgMaxRange);

      expbg[k]->FixParameter(0, fFit[k]->GetParameter(0));
      expbg[k]->FixParameter(1, fFit[k]->GetParameter(1));
      expbg[k]->FixParameter(2, fFit[k]->GetParameter(2));
      expbg[k]->FixParameter(3, fFit[k]->GetParameter(3));
      expbg[k]->FixParameter(4, fFit[k]->GetParameter(4));
      
      bw[k]->FixParameter(0, fFit[k]->GetParameter(5));
      bw[k]->FixParameter(1, fFit[k]->GetParameter(6));
      bw[k]->FixParameter(2, fFit[k]->GetParameter(7));
      ...
      TLegend *legendd = new TLegend(0.16,0.73,0.28,0.89);
      legendd->SetFillColor(4000);
      legendd->SetBorderSize(0);
      legendd->SetTextSize(0.03);

      //legendd->AddEntry(totbg[k], "Tot Bkg", "l");
      legendd->AddEntry(bw[k], "kstar", "l");
      //  legendd->AddEntry(f0bg[k], "f0bw", "l");
      legendd->AddEntry(expbg[k], "exp", "l");
      legendd->Draw("SAME");
