...
void DrawBW()
{
  ...
  for (Int_t i=0; i<nPtBins; i++)
      ...
      for (Int_t j=0; j<nCentBins; j++)
	{
	  ...
	  TLatex* txtInfoB = DrawTLatex(0.526, 0.445, "ALICE Work In Progress", 0.05, 62, 1);
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  fFit[i][j]->SetParLimits(1, 0.725, 0.775);
  ...
  TLatex* txtInfoAAA = DrawTLatex(0.66, 0.845, "ALICE Work In Progress", 0.05, 62, 1);
