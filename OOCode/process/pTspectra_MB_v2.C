...
void DrawBW()
{
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  // Function
	  // ==============
	  fkstar[i][j] = new TF1(Form("kstar_%i_cent_%i",i,j), "[0]*BreitWignerRelativistic(x, [1], [2])", 0.84, 0.93);
	  fBg2[i][j] = new TF1(Form("kstar_GausBg_%i_cent_%i",i,j), "gaus(0)+[3]*BreitWignerRelativistic(x, [4], [5]) + pol2(6)+[12]*TMath::Voigt(x-[13], [14], [15])", BgMinRange, BgMaxRange);
	  fFit[i][j] = new TF1(Form("kstar_fullFit_%i_cent_%i",i,j), "gaus(0)+[3]*BreitWignerRelativistic(x, [4], [5]) + pol2(6) + [9]*BreitWignerRelativistic(x, [10], [11]) + [12]*TMath::Voigt(x-[13], [14], [15])", BgMinRange, BgMaxRange);
	  ...
	  fFit[i][j]->SetParLimits(1, 0.730, 0.775);
	  fFit[i][j]->SetParLimits(2, 0.0363, 0.126);

	  //fFit[i][j]->SetParLimits(3, 0, 90000);
	  fFit[i][j]->FixParameter(4, 0.980);
	  fFit[i][j]->SetParLimits(5, 0.137, 0.187);
	  /* fFit[i][j]->FixParameter(4, 0.97); */
	  /* fFit[i][j]->FixParameter(5, 0.157); */
	  ...
	  fFit[i][j]->FixParameter(13, 1.02); //mass
	  fFit[i][j]->SetParameter(14, 0.048); //width
	  fFit[i][j]->SetParameter(15, 0.048); //width
	  ...
	  phi[i][j] = new TF1(Form("phi_%i_cent_%i",i,j), "[0]*TMath::Voigt(x-[1], [2], [3])", BgMinRange, BgMaxRange);
	  totbg[i][j] = new TF1(Form("totbg_%i_cent_%i",i,j), "gaus(0) + [3]*BreitWignerRelativistic(x, [4], [5]) + pol2(6)+[9]*TMath::Voigt(x-[10], [11], [12])", BgMinRange, BgMaxRange);
	  ...
	  phi[i][j]->FixParameter(0, fFit[i][j]->GetParameter(12));
	  phi[i][j]->FixParameter(1, fFit[i][j]->GetParameter(13));
	  phi[i][j]->FixParameter(2, fFit[i][j]->GetParameter(14));
	  phi[i][j]->FixParameter(3, fFit[i][j]->GetParameter(15));
	  ...
	  totbg[i][j]->FixParameter(9, fFit[i][j]->GetParameter(12));
	  totbg[i][j]->FixParameter(10, fFit[i][j]->GetParameter(13));
	  totbg[i][j]->FixParameter(11, fFit[i][j]->GetParameter(14));
	  totbg[i][j]->FixParameter(12, fFit[i][j]->GetParameter(15));
	  ...
	  covGG_err = covTot.GetSub(0,12,0,12);// these have to be totbg parameters
