...
void DrawBW()
{
  ...
  for (Int_t i=0; i<nPtBins; i++)
      ...
      for (Int_t j=0; j<nCentBins; j++)
	{
	  ...
	  (hUSS_KPi->GetAxis(0))->SetRangeUser(0, 100);
	  (hUSS_PiK->GetAxis(0))->SetRangeUser(0, 100);
	  
	  USSKPi[i][j] = (TH1D*)hUSS_KPi->Projection(2);
	  USSPiK[i][j] = (TH1D*)hUSS_PiK->Projection(2);
	  hUSSKPi[i][j] = (TH1D*)USSKPi[i][j]->Clone(Form("hUSS_KPi%i_%i", i,j));
	  ...
	  (hLSS_KPi->GetAxis(0))->SetRangeUser(0, 100);
	  (hLSS_PiK->GetAxis(0))->SetRangeUser(0, 100);
	  
	  LSSKPi[i][j] = (TH1D*)hLSS_KPi->Projection(2);
	  LSSPiK[i][j] = (TH1D*)hLSS_PiK->Projection(2);
	  hLSSKPi[i][j] = (TH1D*)LSSKPi[i][j]->Clone(Form("hLSS_KPi%i_%i", i,j));
	  hLSSPiK[i][j] = (TH1D*)LSSPiK[i][j]->Clone(Form("hLSS_PiK%i_%i", i,j));     
	  /* hLSS[i][j]->Add(hLSSPiK[i][j]); */
	  /* hLSS[i][j]->Scale(0.5); */

	  hUSSKPi[i][j]->SetName(Form("hUSSKPi_%i_%i", i,j));
	  hUSSPiK[i][j]->SetName(Form("hUSSPiK_%i_%i", i,j));
	  hLSSKPi[i][j]->SetName(Form("hLSSKPi_%i_%i", i,j));
	  hLSSPiK[i][j]->SetName(Form("hLSSPiK_%i_%i", i,j));
	  
	  hCent[i][j] = (TH1D*)hUSS_KPi->Projection(0);
	  hCent[i][j]->SetName(Form("hUSS_Cent_%i_%i", i,j));

	  hUSS_KPi_SUB[i][j] = new TH1D(Form("hUSS_KPi_SUB_%i_%i", i,j), Form("hUSS_KPi_SUB_%i_%i", i,j), nBins, imlow, imtop);
	  hLSS_KPi_SUB[i][j] = new TH1D(Form("hLSS_KPi_SUB_%i_%i", i,j), Form("hLSS_KPi_SUB_%i_%i", i,j), nBins, imlow, imtop);
	  ...
	  hKPiFit[i][j] = new TH1D(Form("hKPiFit_%i_%i", i,j), Form("hKPiFit_%i_%i", i,j), nBins/5, imlow, imtop); 
  ...
  for (Int_t i=0; i<nPtBins; i++)
      ...
      for (Int_t j=0; j<nCentBins; j++)
	{
	  ...
	  Double_t Scale_KPi = hLSSKPi[i][j]->Integral(hLSSKPi[i][j]->FindBin(Nlow), hLSSKPi[i][j]->FindBin(Nhigh))
	    / hUSSKPi[i][j]->Integral(hUSSKPi[i][j]->FindBin(Nlow), hUSSKPi[i][j]->FindBin(Nhigh));

	  double delta;
	  if (i==1||i==2||i==3||i==4||i==6){
	    delta = 0.996; 
	    hLSSKPi[i][j]->Scale(1.0/Scale_KPi*delta);
	  }
	  else if (i==5){
	    delta = 0.98; 
	    hLSSKPi[i][j]->Scale(1.0/Scale_KPi*delta);
	    ...
	    hLSSKPi[i][j]->Scale(1.0/Scale_KPi);

	  
	  HistoAxisTitles(hUSSKPi[i][j], "#it{M}_{ inv}^{ K^{+}#pi^{-}} [GeV/#it{c}^{2}]", "d#it{N}^{ K#pi}/d#it{M}_{ inv}", 0.06, 62, 1.0, 0.06, 62, 1.2);
	  hUSSKPi[i][j]->SetTitle(Form(""));

	  hUSSKPi[i][j]->SetMarkerStyle(4); //circle
	  hUSSKPi[i][j]->SetMarkerSize(1.2);
	  hUSSKPi[i][j]->SetMarkerColor(4);
	  hUSSKPi[i][j]->SetLineColor(4);
	  
	  hLSSKPi[i][j]->SetMarkerStyle(26); //triangle
	  hLSSKPi[i][j]->SetMarkerSize(1.2);
	  hLSSKPi[i][j]->SetMarkerColor(2);
	  hLSSKPi[i][j]->SetLineColor(2);

	  hUSSKPi[i][j]->Rebin(5);
	  hLSSKPi[i][j]->Rebin(5);
      
	  hUSSKPi[i][j]->Scale(0.2);// 1/5
	  hLSSKPi[i][j]->Scale(0.2);// 1/5
	  ...
	  hUSSKPi[i][j]->Draw();
	  hLSSKPi[i][j]->Draw("SAME");
	  TH1D* hShade = (TH1D*)hLSSKPi[i][j]->Clone(Form("hShade_%d_%d", i, j));
	  ...
	  TLatex* txtInfoA = DrawTLatex(0.521, 0.314, "O-O yield, KPi", 0.04, 62, 1);
	  ...
	  legend->AddEntry(hUSSKPi[i][j], "K^{+}#pi^{-}", "lpf");
	  legend->AddEntry(hLSSKPi[i][j], "2#sqrt{#it{M}_{inv}^{ K^{+}#pi^{+}}*#it{M}_{inv}^{ K^{-}#pi^{-}}}", "lpf");
	  legend->Draw("SAME");

	  plots->SaveAs(Form("../kstar-in-oo/Plot/"+localpath+"projKPi_%1.1f_%1.1f_cent_%1.1f_%1.1f.png", ptlow, pthigh, centlow, centhigh),"RECREATE");


	  //============
      	  // PiK histo
	  //============
	  Double_t Scale_PiK = hLSSPiK[i][j]->Integral(hLSSPiK[i][j]->FindBin(Nlow), hLSSPiK[i][j]->FindBin(Nhigh))
	    / hUSSPiK[i][j]->Integral(hUSSPiK[i][j]->FindBin(Nlow), hUSSPiK[i][j]->FindBin(Nhigh));

	  hLSSPiK[i][j]->Scale(1.0/Scale_PiK);
	  ...
	  hLSSPiK[i][j]->SetMarkerStyle(26); //triangle
	  hLSSPiK[i][j]->SetMarkerSize(1.2);
	  hLSSPiK[i][j]->SetMarkerColor(2);
	  hLSSPiK[i][j]->SetLineColor(2);

	  hUSSPiK[i][j]->Rebin(5);
	  hLSSPiK[i][j]->Rebin(5);
      
	  hUSSPiK[i][j]->Scale(0.2);// 1/5
	  hLSSPiK[i][j]->Scale(0.2);// 1/5
	  ...
	  hUSSPiK[i][j]->Draw();
	  hLSSPiK[i][j]->Draw("SAME");
	  TH1D* hShade2 = (TH1D*)hLSSPiK[i][j]->Clone(Form("hShade_%d_%d", i, j));
	  ...
	  legend2->AddEntry(hLSSPiK[i][j], "2#sqrt{#it{M}_{inv}^{ K^{+}#pi^{+}}*#it{M}_{inv}^{ K^{-}#pi^{-}}}", "lpf");
	  legend2->Draw("SAME");
	  ...
	  // ==========================
	  hUSS_KPi_SUB[i][j] = (TH1D*)hUSSKPi[i][j]->Clone(Form("hUSS_Minv_%1.1f_%1.1f_cent_%1.1f_%1.1f", ptlow, pthigh, 0.0, 100.0));
	  hLSS_KPi_SUB[i][j] = (TH1D*)hLSSKPi[i][j]->Clone(Form("hLSS_Minv_%1.1f_%1.1f_cent_%1.1f_%1.1f", ptlow, pthigh, 0.0, 100.0));
	  hUSS_KPi_SUB[i][j]->Add(hLSS_KPi_SUB[i][j], -1);

	  hUSS_PiK_SUB[i][j] = (TH1D*)hUSSPiK[i][j]->Clone(Form("hUSS_Minv_%1.1f_%1.1f_cent_%1.1f_%1.1f", ptlow, pthigh, 0.0, 100.0));
	  hLSS_PiK_SUB[i][j] = (TH1D*)hLSSPiK[i][j]->Clone(Form("hLSS_Minv_%1.1f_%1.1f_cent_%1.1f_%1.1f", ptlow, pthigh, 0.0, 100.0));
  ...
  for (int i=0; i<nPtBins; i++)
      ...
      for (int j=0; j<nCentBins; j++)
	{
	  ...
	  // KPi histo
	  //============
	  hKPiFit[i][j]->Add(hUSS_KPi_SUB[i][j]);
	  ...
	  fFit[i][j]->SetParLimits(0, 0, 8000);
	  fFit[i][j]->SetParLimits(1, 0.770, 0.820);
	  fFit[i][j]->SetParLimits(2, 0.050, 0.120);	  

	  fFit[i][j]->SetParLimits(3, 0, 20000);
	  fFit[i][j]->SetParLimits(4, 0.960, 1.03);
	  ...
	  hKPiFit[i][j]->Fit(fFit[i][j], "SR", "", BgMinRange, BgMaxRange);
	  hKPiFit[i][j]->Fit(fFit[i][j], "SR", "", BgMinRange, BgMaxRange);
	  hKPiFit[i][j]->SetTitle(Form(""));
	  hKPiFit[i][j]->Draw();
	  ...
	  TFitResultPtr ptr = hKPiFit[i][j]->Fit(fFit[i][j], "SR0", "", BgMinRange, BgMaxRange);
	  ...
	  HistoAxisTitles(hKPiFit[i][j], "#it{M}_{ inv}^{ K^{+}#pi^{-} }[GeV/#it{c}^{2}]",
	  ...
	  TLatex* txtInfoBB = DrawTLatex(0.173, 0.873, "O-O yield, KPi", 0.04, 62, 1);
	  ...
	  lines->SaveAs(Form("../kstar-in-oo/Plot/"+localpath+"KPi_fit_%1.1f_%1.1f_cent_%1.1f_%1.1f.png", ptlow, pthigh, centlow, centhigh), "RECREATE");
	  ...
	  double HistY = 0;
	  double HistYerr = 0;
	  double lowM = hKPiFit[i][j]->FindBin(0.85);
	  double highM = hKPiFit[i][j]->FindBin(0.95);
	  HistY=hKPiFit[i][j]->IntegralAndError(lowM, highM, HistYerr);
	  double binW = hKPiFit[i][j]->GetBinWidth(50); //Any number
