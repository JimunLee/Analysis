...
void DrawBW()
{
  ...
  TString localpath = "Data/Inside/TrackHighPt/";
  //===================
  // Data file inputs 
  //===================
  ...
  TString USSKPiName("kstar-in-o-o_id47423/hUSS_KPi_INSIDE;1");
  TString USSPiKName("kstar-in-o-o_id47423/hUSS_PiK_INSIDE;1");
  TString LSSKPiName("kstar-in-o-o_id47423/hLSS_KPi_INSIDE;1");
  TString LSSPiKName("kstar-in-o-o_id47423/hLSS_PiK_INSIDE;1");
  TString CentName("kstar-in-o-o_id47423/nEvents;1");
  //============================================================================
  ...
  for (Int_t i=0; i<nPtBins; i++)
	...
	hLSS[i]->Scale(1.0/Scale*delta);
      ...
      hUSS[i]->Scale(1/ReBinNum);
      hLSS[i]->Scale(1/ReBinNum);
	...
	if (bin < binLow || bin > binHigh) {
      ...
      TLatex* txtInfoD = DrawTLatex(0.522, 0.216, "Track High pT", 0.04, 42, 1);
  ...
  for (int i=0; i<nPtBins; i++)
    {
      ...
      TLatex* txtInfoBB = DrawTLatex(0.173, 0.873, "TrackHighPt", 0.04, 62, 1);
  ...
  TFile* fout = TFile::Open("../kstar-in-jets/Results_draw/JetUncorrpTspectra_trg3_TrackHighPt.root", "RECREATE");
  fout->cd();
  ...
  hYield->SetLineColor(green);
  hYield->SetMarkerStyle(43); //circle
  hYield->SetMarkerSize(3);
  hYield->SetMarkerColor(green);
  hYield->Draw("SAME");

  TLatex* txtInfoAAA = DrawTLatex(0.58, 0.845, "ALICE Work In Progress", 0.05, 62, 1);
  TLatex* txtInfoBBB = DrawTLatex(0.59, 0.789, "TrackHighPt, #sqrt{s} = 13.6 TeV", 0.05, 42, 1);
  ...
  numbin->SetLogy();
  numbin->SaveAs("../kstar-in-jets/Plot/kstarInjets/"+localpath+"h_Yield_trg3_TrackHighPt.png", "RECREATE");
}//DrawBW
