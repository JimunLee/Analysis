...
void DrawBW()
{
  ...
  // Data file inputs 
  //===================
  TString mainfile = "../kstar-in-jets/Results_rootfile/MB_ao_pass1_skimmed_merged.root";
  //TString mainfile = "../kstar-in-jets/Results_rootfile/MB_ao_MinBias.root";
  TFile* data = TFile::Open(mainfile);
  if(!data)   return;

  TString USSKPiName("kstar-in-o-o/hMB_USS_KPi;1");
  TString USSPiKName("kstar-in-o-o/hMB_USS_PiK;1");
  TString LSSKPiName("kstar-in-o-o/hMB_LSS_KPi;1");
  TString LSSPiKName("kstar-in-o-o/hMB_LSS_PiK;1");
  TString CentName("kstar-in-o-o/nEvents;1");
  //============================================================================
  ...
  for (Int_t i=0; i<nPtBins; i++)
	...
	if (bin < binLow || bin > binHigh) {
      ...
      TLatex* txtInfoA = DrawTLatex(0.521, 0.318, "MB", 0.04, 62, 1);
  ...
  for (int i=0; i<nPtBins; i++)
    {
      ...
      TLatex* txtInfoBB = DrawTLatex(0.173, 0.873, "MB", 0.04, 62, 1);
  ...
  TFile* fout = TFile::Open("../kstar-in-jets/Results_draw/MBuncorrectedpTspectra_skimmed.root", "RECREATE");
  hYield->Write("MBspectra");
  fout->Write();
  fout->Close();
  ...
  TLatex* txtInfoBBB = DrawTLatex(0.59, 0.789, "MB, #sqrt{s} = 13.6 TeV", 0.05, 42, 1);
