...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (int k=1; k<nPtBins+1; k++)
    {
      hUSS_Y_kstarSig[nPtBins-k]->Add(hUSS_Y_SUB[nPtBins-k]);
      hUSS_Y_Bg[nPtBins-k]->Add(hUSS_Y_SUB[nPtBins-k]);
      hUSS_Y_ExtraBg[nPtBins-k]->Add(hUSS_Y_SUB[nPtBins-k]);
      
      hUSS_Y_Fit[nPtBins-k]->Add(hUSS_Y_SUB[nPtBins-k]);


      for (int bin = hUSS_Y_Bg[nPtBins-k]->FindBin(0.85); bin <= hUSS_Y_Bg[nPtBins-k]->FindBin(0.93); bin++)
	hUSS_Y_Bg[nPtBins-k]->SetBinError(bin, 0);
	
      for (int bin = hUSS_Y_ExtraBg[nPtBins-k]->FindBin(0.85); bin <= hUSS_Y_ExtraBg[nPtBins-k]->FindBin(0.95); bin++)
	hUSS_Y_ExtraBg[nPtBins-k]->SetBinError(bin, 0);
      
      for (int bin = hUSS_Y_kstarSig[nPtBins-k]->FindBin(0.76); bin <= hUSS_Y_kstarSig[nPtBins-k]->FindBin(0.84); bin++)
	hUSS_Y_kstarSig[nPtBins-k]->SetBinError(bin, 0);
      ...
      // ==============
      // 1. Extra fit
      fBg[nPtBins-k] = new TF1(Form("kstar_ExtraBg_%i",nPtBins-k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x)", ExtraMinRange, BgMaxRange);
      hUSS_Y_Bg[nPtBins-k]->Fit(fBg[nPtBins-k], "0", "", ExtraMinRange, BgMaxRange);
      
      // 2. Background fit
      fExtraBg[nPtBins-k] = new TF1(Form("kstar_bg_%i",nPtBins-k), "pol4", ExtraMinRange, BgMaxRange);
      hUSS_Y_ExtraBg[nPtBins-k]->Fit(fExtraBg[nPtBins-k], "0", "", ExtraMinRange, BgMaxRange);

      // 3. kstar
      fkstar[nPtBins-k] = new TF1(Form("kstar_%i",nPtBins-k), "[0]*BreitWignerRelativistic(x, [1], [2])", kstarMinRange, kstarMaxRange);
      fkstar[nPtBins-k]->SetParameter(1, kstarMass);
      fkstar[nPtBins-k]->SetParameter(2, 0.0474);
      hUSS_Y_kstarSig[nPtBins-k]->Fit(fkstar[nPtBins-k], "0", "", ExtraMinRange, BgMaxRange);
	...
	fFit[nPtBins-k] = new TF1(Form("Fullfit_%i", nPtBins-k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4) + [9]*BreitWignerRelativistic(x, [10], [11])", ExtraMinRange, BgMaxRange);
	
	fFit[nPtBins-k]->SetParameter(0, fBg[nPtBins-k]->GetParameter(0));
	fFit[nPtBins-k]->SetParameter(1, fBg[nPtBins-k]->GetParameter(1));
	fFit[nPtBins-k]->SetParameter(2, fBg[nPtBins-k]->GetParameter(2));
	fFit[nPtBins-k]->SetParameter(3, fBg[nPtBins-k]->GetParameter(3));
	
	fFit[nPtBins-k]->SetParameter(4, fExtraBg[nPtBins-k]->GetParameter(0));
	fFit[nPtBins-k]->SetParameter(5, fExtraBg[nPtBins-k]->GetParameter(1));
	fFit[nPtBins-k]->SetParameter(6, fExtraBg[nPtBins-k]->GetParameter(2));
	fFit[nPtBins-k]->SetParameter(7, fExtraBg[nPtBins-k]->GetParameter(3));
	fFit[nPtBins-k]->SetParameter(8, fExtraBg[nPtBins-k]->GetParameter(4));
			      
	fFit[nPtBins-k]->SetParameter(9, fkstar[nPtBins-k]->GetParameter(0));
	fFit[nPtBins-k]->SetParameter(10, fkstar[nPtBins-k]->GetParameter(1));
	fFit[nPtBins-k]->SetParameter(11, fkstar[nPtBins-k]->GetParameter(2));
      }
      
      else{
	fFit[nPtBins-k] = new TF1(Form("Fullfit_%i", nPtBins-k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4) + [9]*BreitWignerRelativistic(x, [10], [11])", ExtraMinRange, BgMaxRange);
	
	fFit[nPtBins-k]->SetParameter(0, fBg_POST[nPtBins-k+1]->GetParameter(0));
	fFit[nPtBins-k]->SetParameter(1, fBg_POST[nPtBins-k+1]->GetParameter(1));
	fFit[nPtBins-k]->SetParameter(2, fBg_POST[nPtBins-k+1]->GetParameter(2));
	fFit[nPtBins-k]->SetParameter(3, fBg_POST[nPtBins-k+1]->GetParameter(3));
	fFit[nPtBins-k]->SetParameter(4, fBg_POST[nPtBins-k+1]->GetParameter(4));
	fFit[nPtBins-k]->SetParameter(5, fBg_POST[nPtBins-k+1]->GetParameter(5));
	fFit[nPtBins-k]->SetParameter(6, fBg_POST[nPtBins-k+1]->GetParameter(6));
	fFit[nPtBins-k]->SetParameter(7, fBg_POST[nPtBins-k+1]->GetParameter(7));
	fFit[nPtBins-k]->SetParameter(8, fBg_POST[nPtBins-k+1]->GetParameter(8));
	fFit[nPtBins-k]->SetParameter(9, fkstar_POST[nPtBins-k+1]->GetParameter(0));
	fFit[nPtBins-k]->SetParameter(10, fkstar_POST[nPtBins-k+1]->GetParameter(1));
	fFit[nPtBins-k]->SetParameter(11, fkstar_POST[nPtBins-k+1]->GetParameter(2));
      }
      
      hUSS_Y_Fit[nPtBins-k]->Fit(fFit[nPtBins-k], "SR0", "", ExtraMinRange, BgMaxRange);
      hUSS_Y_Fit[nPtBins-k]->Fit(fFit[nPtBins-k], "SR0", "", ExtraMinRange, BgMaxRange);

      // =================================================================================
      // for fitting each onea
      fBg_POST[nPtBins-k] = new TF1(Form("kstar_ExtraBg_POST_%i",nPtBins-k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4)", ExtraMinRange, BgMaxRange);
      fBg_POST[nPtBins-k]->SetParameter(0, fFit[nPtBins-k]->GetParameter(0));
      fBg_POST[nPtBins-k]->SetParameter(1, fFit[nPtBins-k]->GetParameter(1));
      fBg_POST[nPtBins-k]->SetParameter(2, fFit[nPtBins-k]->GetParameter(2));
      fBg_POST[nPtBins-k]->SetParameter(3, fFit[nPtBins-k]->GetParameter(3));
      fBg_POST[nPtBins-k]->SetParameter(4, fFit[nPtBins-k]->GetParameter(4));
      fBg_POST[nPtBins-k]->SetParameter(5, fFit[nPtBins-k]->GetParameter(5));
      fBg_POST[nPtBins-k]->SetParameter(6, fFit[nPtBins-k]->GetParameter(6));
      fBg_POST[nPtBins-k]->SetParameter(7, fFit[nPtBins-k]->GetParameter(7));
      fBg_POST[nPtBins-k]->SetParameter(8, fFit[nPtBins-k]->GetParameter(8));

      fkstar_POST[nPtBins-k] = new TF1(Form("kstar_kstar_POST_%i",nPtBins-k), "[0]*BreitWignerRelativistic(x, [1], [2])", ExtraMinRange, BgMaxRange);
      fkstar_POST[nPtBins-k]->SetParameter(0, fFit[nPtBins-k]->GetParameter(9));
      fkstar_POST[nPtBins-k]->SetParameter(1, fFit[nPtBins-k]->GetParameter(10));
      fkstar_POST[nPtBins-k]->SetParameter(2, fFit[nPtBins-k]->GetParameter(11));

      TFitResultPtr ptr = hUSS_Y_Fit[nPtBins-k]->Fit(fFit[nPtBins-k], "SR0", "", ExtraMinRange, BgMaxRange);
      ...
      fFit[nPtBins-k]->SetLineWidth(4);
      fFit[nPtBins-k]->Draw("SAME");

      // background
      fBg_POST[nPtBins-k]->SetLineColor(4);
      fBg_POST[nPtBins-k]->SetLineWidth(4);
      fBg_POST[nPtBins-k]->Draw("SAME");

      // kstar
      fkstar_POST[nPtBins-k]->SetLineColor(6);
      fkstar_POST[nPtBins-k]->SetLineWidth(4);
      fkstar_POST[nPtBins-k]->Draw("SAME");

      /*  // background */
      /* fBg[nPtBins-k]->SetLineColor(7); */
      /* fBg[nPtBins-k]->SetLineWidth(4); */
      /* fBg[nPtBins-k]->Draw("SAME"); */

      /* // kstar */
      /* fkstar[nPtBins-k]->SetLineColor(9); */
      /* fkstar[nPtBins-k]->SetLineWidth(4); */
      /* fkstar[nPtBins-k]->Draw("SAME"); */
