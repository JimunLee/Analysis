  ...
  for (int k=1; k<nPtBins+1; k++)
    {
      ...
      //omega
      fBW_omega[nPtBins-k] = new TF1(Form("omega_bw_%i", nPtBins-k), "[0]*BreitWigner(x, [1], [2])", omegaMinRange, omegaMaxRange);
      ...
      // =====================
      // fitting background
      fBg[nPtBins-k] = new TF1(Form("BgFit_%i", nPtBins-k), "pol2(0)", Bglow, Bghigh);
      ...
      fFit[nPtBins-k] = new TF1(Form("Fullfit_%i", nPtBins-k), "[0]*BreitWigner(x, [1], [2]) + [3]*BreitWigner(x, [4], [5]) + pol2(6)", Bglow, Bghigh);
