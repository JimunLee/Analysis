...
void DrawVoigt(TString Kind, TString USS, TString LSS)
{
  ...
  Double_t kstarMinRange = 0.76;
  Double_t kstarMaxRange = 1.04;
  
  Double_t rhoMinRange = 0.76;
  ...
  /* lines->Divide(3,3); */
  for(int fpt=1; fpt<nPtBins; fpt++) //nPtBins+1 MUST DELETE +! When i wanna draw one histogram
    {
      if(fpt>2)
	continue;
      
      hUSS_Y_Bkg[nPtBins-fpt]->Add(hUSS_Y[nPtBins-fpt]);
      hUSS_Y_Peak[nPtBins-fpt]->Add(hUSS_Y[nPtBins-fpt]);
      
      for(int bin = hUSS_Y_Bkg[nPtBins-fpt]->FindBin(kstarMinRange); bin<=hUSS_Y_Bkg[nPtBins-fpt]->FindBin(kstarMaxRange); bin++)
	hUSS_Y_Bkg[nPtBins-fpt]->SetBinError(bin,0);
      
      ptlow = xBins[nPtBins-fpt];
      pthigh = xBins[nPtBins-fpt+1];
      
      //=========================================================================
      // Peak Function of BreitWigner         | Gaus                            |
      //=========================================================================
      //| Information : K*                    | Information : rho0(770)         | 
      //| gamma(1,3) : 0.0474                 | mass(1) : 0.77526               | 
      //| mass(2) : 0.89166                   | gamma(2) : 0.1478               |  
      //=========================================================================
      fBW_kstar[nPtBins-fpt] = new TF1(Form("BW_kstar_fit_%i",nPtBins-fpt),"[0]*TMath::BreitWigner(x, [1], [2])", kstarMinRange, kstarMaxRange);
      fBW_kstar[nPtBins-fpt]->SetLineColor(2); //Red:2

      fBW_kstar[nPtBins-fpt]->SetParameter(0,1.0);
      fBW_kstar[nPtBins-fpt]->SetParameter(1,0.89166);
      fBW_kstar[nPtBins-fpt]->SetParameter(2,0.0474);

      //Information of omega(782)
      fBW_rho[nPtBins-fpt] = new TF1(Form("BW_rho_fit_%i",nPtBins-fpt),"[0]*TMath::BreitWigner(x, [1], [2])", rhoMinRange, rhoMaxRange);
      fBW_rho[nPtBins-fpt]->SetLineColor(2); //Red:2
      
      // fBW_rho[nPtBins-fpt]->SetParameter(3,1.0);
      fBW_rho[nPtBins-fpt]->SetParameter(1,0.78265);
      fBW_rho[nPtBins-fpt]->FixParameter(2,0.0849);
      
      //add
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_kstar[nPtBins-fpt], "0", "", kstarMinRange, kstarMaxRange);
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_kstar[nPtBins-fpt], "0", "", kstarMinRange, kstarMaxRange);
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_kstar[nPtBins-fpt], "0", "", kstarMinRange, kstarMaxRange);
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_kstar[nPtBins-fpt], "0", "", kstarMinRange, kstarMaxRange);
      
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_rho[nPtBins-fpt], "0", "", rhoMinRange, rhoMaxRange);
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_rho[nPtBins-fpt], "0", "", rhoMinRange, rhoMaxRange);
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_rho[nPtBins-fpt], "0", "", rhoMinRange, rhoMaxRange);
      hUSS_Y_Peak[nPtBins-fpt]->Fit(fBW_rho[nPtBins-fpt], "0", "", rhoMinRange, rhoMaxRange);
      
      // fBreitCopy[nPtBins-fpt] = new TF1(Form("BwFit_Copy_%i",nPtBins-fpt),"[0]*TMath:::BreitWigner(x, [1], [2])", Siglow, Sighigh);
      // fBreitCopy[nPtBins-fpt]->SetLineColor(2);

      //=========================================
      // Background Function
      //=========================================
      fBg[nPtBins-fpt] = new TF1(Form("BkgFit_%i",nPtBins-fpt), "pol3", Bglow, Bghigh); //you have to be careful! This is 4 par

      fBg[nPtBins-fpt]->SetLineColor(1); //Black:1
      fBg[nPtBins-fpt]->SetLineStyle(2);
      fBg[nPtBins-fpt]->SetLineWidth(4);
      fBg[nPtBins-fpt]->SetRange(Bglow, Bghigh);

      hUSS_Y_Bkg[nPtBins-fpt]->Fit(fBg[nPtBins-fpt], "0", "", Bglow, Bghigh);
      hUSS_Y_Bkg[nPtBins-fpt]->Fit(fBg[nPtBins-fpt], "0", "", Bglow, Bghigh);

      //=========================================
      // Combine Peak + Background : Main
      //=========================================
      fFit[nPtBins-fpt] = new TF1(Form("FullFit_%i",nPtBins-fpt), "[0]*TMath::BreitWigner(x, [1], [2]) + [3]*TMath::BreitWigner(x, [4], [5]) + pol3(6)", Bglow, Bghigh);
      fFit[nPtBins-fpt]->SetLineColor(2);

      // SIGNAL
      fFit[nPtBins-fpt]->SetParameter(0, fBW_kstar[nPtBins-fpt]->GetParameter(0));
      fFit[nPtBins-fpt]->SetParameter(1, fBW_kstar[nPtBins-fpt]->GetParameter(1));
      fFit[nPtBins-fpt]->SetParameter(2, fBW_kstar[nPtBins-fpt]->GetParameter(2));

      fFit[nPtBins-fpt]->SetParameter(3, fBW_rho[nPtBins-fpt]->GetParameter(0));
      fFit[nPtBins-fpt]->SetParameter(4, fBW_rho[nPtBins-fpt]->GetParameter(1));
      fFit[nPtBins-fpt]->SetParameter(5, fBW_rho[nPtBins-fpt]->GetParameter(2));


      if(fpt==2){
	// SIGNAL
	//	int getter = fpt-1;
	fFit[nPtBins-fpt]->SetParameter(0, fFit[8]->GetParameter(0));
	fFit[nPtBins-fpt]->SetParameter(1, fFit[8]->GetParameter(1));
	fFit[nPtBins-fpt]->SetParameter(2, fFit[8]->GetParameter(2));
	fFit[nPtBins-fpt]->SetParameter(3, fFit[8]->GetParameter(3));
	fFit[nPtBins-fpt]->SetParameter(4, fFit[8]->GetParameter(4));
	fFit[nPtBins-fpt]->SetParameter(5, fFit[8]->GetParameter(5));
	fFit[nPtBins-fpt]->SetParameter(6, fFit[8]->GetParameter(6));
	fFit[nPtBins-fpt]->SetParameter(7, fFit[8]->GetParameter(7));
	fFit[nPtBins-fpt]->SetParameter(8, fFit[8]->GetParameter(8));
	fFit[nPtBins-fpt]->SetParameter(9, fFit[8]->GetParameter(9));
	//fix GAMMA for rho and Kstar
      }
 
      // BACKGROUND
      fFit[nPtBins-fpt]->SetParameter(6, fBg[nPtBins-fpt]->GetParameter(0));
      fFit[nPtBins-fpt]->SetParameter(7, fBg[nPtBins-fpt]->GetParameter(1));
      fFit[nPtBins-fpt]->SetParameter(8, fBg[nPtBins-fpt]->GetParameter(2));
      fFit[nPtBins-fpt]->SetParameter(9, fBg[nPtBins-fpt]->GetParameter(3));
      
      // OK, WE NOW HAVE GOOD STARTING VALUES
      hUSS_Y[nPtBins-fpt]->Fit(fFit[nPtBins-fpt], "SR0", "", Bglow, Bghigh);
      hUSS_Y[nPtBins-fpt]->Fit(fFit[nPtBins-fpt], "SR0", "", Bglow, Bghigh);
      hUSS_Y[nPtBins-fpt]->Fit(fFit[nPtBins-fpt], "SR0", "", Bglow, Bghigh);
      
      fBg[nPtBins-fpt]->SetRange(Bglow, Bghigh);

      fBW_kstar[nPtBins-fpt]->SetParameters(fFit[nPtBins-fpt]->GetParameters());
      fBW_rho[nPtBins-fpt]->SetParameters(fFit[nPtBins-fpt]->GetParameters());
      //  fBreitCopy[nPtBins-fpt]->SetParameters(fFit[nPtBins-fpt]->GetParameters());

      //==========================
      // Draw
      //==========================
      TCanvas * lines = new TCanvas(Form("line_%d",nPtBins-fpt),"" , 940, 800);

      lines->SetLeftMargin(0.12);
      lines->SetRightMargin(0.1);
      lines->SetTopMargin(0.08);
      lines->SetBottomMargin(0.14);
      // lines->cd(nPtBins-fpt +1);

      hUSS_Y_COPY[nPtBins-fpt] = (TH1D*)hUSS_Y[nPtBins-fpt]->Clone(Form("hUSS_Y_Minv_%1.1f_%1.1f", ptlow, pthigh));
      hLSS_Y_COPY[nPtBins-fpt] = (TH1D*)hLSS_Y[nPtBins-fpt]->Clone(Form("hLSS_Y_Minv_%1.1f_%1.1f", ptlow, pthigh));
      
      hUSS_Y_COPY[nPtBins-fpt]->GetXaxis()->SetTitle("#it{M}_{ inv}^{ K^{+}#pi^{-} }[GeV/#it{c}^{2}]");
      hUSS_Y_COPY[nPtBins-fpt]->GetXaxis()->SetTitleSize(0.06);
      hUSS_Y_COPY[nPtBins-fpt]->GetXaxis()->SetTitleFont(62);
      hUSS_Y_COPY[nPtBins-fpt]->GetXaxis()->SetTitleOffset(1.0);
      
      hUSS_Y_COPY[nPtBins-fpt]->GetYaxis()->SetTitle("1/#it{N}_{ ev} d#it{N}^{ K^{+}#pi^{-}}_{USS-LSS}/d#it{M}_{ inv}");
      hUSS_Y_COPY[nPtBins-fpt]->GetYaxis()->SetTitleSize(0.04);
      hUSS_Y_COPY[nPtBins-fpt]->GetYaxis()->SetTitleOffset(1.50);
      hUSS_Y_COPY[nPtBins-fpt]->GetYaxis()->SetTitleFont(62);
      hUSS_Y_COPY[nPtBins-fpt]->GetYaxis()->SetTitleOffset(1.0);
      hUSS_Y_COPY[nPtBins-fpt]->SetTitle(Form(""));
      hUSS_Y_COPY[nPtBins-fpt]->SetTitleSize(0.05, "t");

      hUSS_Y_COPY[nPtBins-fpt]->SetLineColor(1);
      hUSS_Y_COPY[nPtBins-fpt]->Draw();

      fFit[nPtBins-fpt]->SetLineColor(2);
      fFit[nPtBins-fpt]->Draw("SAME");

      fBg[nPtBins-fpt]->SetParameter(0, fFit[nPtBins-fpt]->GetParameter(6));
      fBg[nPtBins-fpt]->SetParameter(1, fFit[nPtBins-fpt]->GetParameter(7));
      fBg[nPtBins-fpt]->SetParameter(2, fFit[nPtBins-fpt]->GetParameter(8));
      fBg[nPtBins-fpt]->SetParameter(3, fFit[nPtBins-fpt]->GetParameter(9));

      fBg[nPtBins-fpt]->SetLineColor(4);
      fBg[nPtBins-fpt]->Draw("SAME");

      TFitResultPtr ptr = hUSS_Y[nPtBins-fpt]->Fit(fFit[nPtBins-fpt], "SR0", "", Bglow, Bghigh);
	...
	SpecialLatex_fit2->Draw("SAME");
      ...
      legendd->AddEntry(hUSS_Y_COPY[nPtBins-fpt],"Data", "lep");
      legendd->AddEntry(fBg[nPtBins-fpt], "Bkg. Fit", "l");
      legendd->AddEntry(fFit[nPtBins-fpt], "Voigt. Fit", "l");
      legendd->Draw("SAME");
