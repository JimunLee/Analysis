  ...
  for (int k=0; k<nPtBins; k++)
    {
      ...
      for (int bin = hUSS_Y_Bg[k]->FindBin(0.85); bin <= hUSS_Y_Bg[k]->FindBin(0.93); bin++)
	...
	hUSS_Y_ExtraBg[k]->SetBinError(bin, 0);
      ...
      for (int bin = hUSS_Y_kstarSig[k]->FindBin(0.76); bin <= hUSS_Y_kstarSig[k]->FindBin(0.84); bin++)
