...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  // Jet
	...
	for (int bin = hUSS_Y_Bg[k]->FindBin(0.79); bin <= hUSS_Y_Bg[k]->FindBin(0.96); bin++)
	  hUSS_Y_Bg[k]->SetBinError(bin, 0);
	
	for (int bin = hUSS_Y_kstarSig[k]->FindBin(0.765); bin <= hUSS_Y_kstarSig[k]->FindBin(0.84); bin++)
	  hUSS_Y_kstarSig[k]->SetBinError(bin, 0);
	...
	fBg[k]->SetLineColor(7);
	fBg[k]->SetLineWidth(4);
	fBg[k]->Draw("SAME");
	/* fkstar[k]->SetLineColor(1); */
	...
	double HistY = 0;
	double HistYerr = 0;
	double lowM = hUSS_Y_Fit[k]->FindBin(0.86);
	double highM = hUSS_Y_Fit[k]->FindBin(0.92);
	HistY=hUSS_Y_Fit[k]->IntegralAndError(lowM, highM, HistYerr);
	double binW = hUSS_Y_Fit[k]->GetBinWidth(50); //Any number
	double BgY = fBg_POST[k]->Integral(0.86,0.92)/binW;
	...
	covTot = ptr->GetCovarianceMatrix();
	covGG_err = covTot.GetSub(0,8,0,8);// these have to be fBG parameters,
	Double_t Bg_err = (1/binW)*(fBg_POST[k]->IntegralError(0.86,0.92,fBg_POST[k]->GetParameters(),covGG_err.GetMatrixArray()));
    ...
    /* 	/\* for (int bin = hUSS_Y_Bg[k]->FindBin(0.845); bin <= hUSS_Y_Bg[k]->FindBin(0.99); bin++) *\/ */
    /* 	/\*   hUSS_Y_Bg[k]->SetBinError(bin, 0); *\/ */

    /* 	/\* //0.78~1.05 *\/ */
    /*  	/\* for (int bin = hUSS_Y_ExtraBg[k]->FindBin(0.78); bin <= hUSS_Y_ExtraBg[k]->FindBin(1.01); bin++) *\/ */
    /* 	/\*   hUSS_Y_ExtraBg[k]->SetBinError(bin, 0); *\/ */

    /* 	/\* for (int bin = hUSS_Y_kstarSig[k]->FindBin(0.79); bin <= hUSS_Y_kstarSig[k]->FindBin(0.95); bin++) *\/ */
    /* 	/\*   hUSS_Y_kstarSig[k]->SetBinError(bin, 0); *\/ */
