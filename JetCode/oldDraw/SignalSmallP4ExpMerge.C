...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (int k=1; k<nPtBins+1; k++)
	...
	continue;
      ...
      // exponential
      for (int bin = hUSS_Y_Bg[k]->FindBin(0.84); bin <= hUSS_Y_Bg[k]->FindBin(1.05); bin++)
	hUSS_Y_Bg[k]->SetBinError(bin, 0);

      // first bump : pol4
      for (int bin = hUSS_Y_ExtraBg[k]->FindBin(0.84); bin <= hUSS_Y_ExtraBg[k]->FindBin(1.05); bin++)
	hUSS_Y_ExtraBg[k]->SetBinError(bin, 0);

      // second bump : pol4
      for (int bin = hUSS_Y_Bgtwo[k]->FindBin(0.77); bin <= hUSS_Y_Bgtwo[k]->FindBin(0.94); bin++)
	hUSS_Y_Bgtwo[k]->SetBinError(bin, 0);

      // kstar
      for (int bin = hUSS_Y_kstarSig[k]->FindBin(0.76); bin <= hUSS_Y_kstarSig[k]->FindBin(0.825); bin++)
	hUSS_Y_kstarSig[k]->SetBinError(bin, 0);
      ...
      // 3. Bgtwo fit
      fExtraBgtwo[k] = new TF1(Form("kstar_bgtwo_%i",k), "pol4", ExtraMinRange, BgMaxRange);
      hUSS_Y_Bgtwo[k]->Fit(fExtraBgtwo[k], "0", "", ExtraMinRange, BgMaxRange);

      // 4. kstar
      fkstar[k] = new TF1(Form("kstar_%i",k), "[0]*BreitWignerRelativistic(x, [1], [2])", kstarMinRange, kstarMaxRange);
      fkstar[k]->SetParameter(1, 0.89581);
	...
	fFit[k] = new TF1(Form("Fullfit_%i", k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4) + pol4(9) + [14]*BreitWignerRelativistic(x, [15], [16])", ExtraMinRange, BgMaxRange);
	...
	fFit[k]->SetParameter(9, fExtraBgtwo[k]->GetParameter(0));
	fFit[k]->SetParameter(10, fExtraBgtwo[k]->GetParameter(1));
	fFit[k]->SetParameter(11, fExtraBgtwo[k]->GetParameter(2));
	fFit[k]->SetParameter(12, fExtraBgtwo[k]->GetParameter(3));
	fFit[k]->SetParameter(13, fExtraBgtwo[k]->GetParameter(4));

	fFit[k]->SetParameter(14, fkstar[k]->GetParameter(0));
	fFit[k]->SetParameter(15, fkstar[k]->GetParameter(1));
	fFit[k]->SetParameter(16, fkstar[k]->GetParameter(2));
      }  
      ...
      // First
      /* fBg_POST[k] = new TF1(Form("kstar_ExtraBg_POST_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x)", ExtraMinRange, BgMaxRange); */
      ...
      fBg_POST[k] = new TF1(Form("kstar_ExtraBg_POST_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4) + pol4(9)", ExtraMinRange, BgMaxRange);
      ...
      fBg_POST[k]->SetParameter(9, fFit[k]->GetParameter(9));
      fBg_POST[k]->SetParameter(10, fFit[k]->GetParameter(10));
      fBg_POST[k]->SetParameter(11, fFit[k]->GetParameter(11));
      fBg_POST[k]->SetParameter(12, fFit[k]->GetParameter(12));
      fBg_POST[k]->SetParameter(13, fFit[k]->GetParameter(13));

      // Fourth
      fkstar_POST[k] = new TF1(Form("kstar_%i",k), "[0]*BreitWignerRelativistic(x, [1], [2])", ExtraMinRange, BgMaxRange);
      
      fkstar_POST[k]->SetParameter(0, fFit[k]->GetParameter(14));
      fkstar_POST[k]->SetParameter(1, fFit[k]->GetParameter(15)); 
      fkstar_POST[k]->SetParameter(2, fFit[k]->GetParameter(16));
      ...
      /* // background */
      /* fExtraBgtwo_POST[k]->SetLineColor(2); */
      /* fExtraBgtwo_POST[k]->SetLineWidth(4); */
      /* fExtraBgtwo_POST[k]->Draw("SAME");    */   
