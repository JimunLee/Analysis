...
void DrawVoigt(TString Kind, TString USS, TString LSS)
{
  ...
  if (Kind == "Outside"){
    ...
    SpecialLatex_plot = DrawTLatex(0.659, 0.772, "Min. Bias", 0.04, 42, 1);
    SpecialLatex_fit = DrawTLatex(0.557, 0.79, "Min. Bias", 0.04, 42, 1);  
  }
  else if (Kind == "Inside"){
  ...
  for(Int_t ipt=0; ipt<nPtBins; ipt++)
	...
	/ hLSS_Y[ipt]->Integral(hLSS_Y[ipt]->FindBin(Nlow), hLSS_Y[ipt]->FindBin(Nhigh));
      ...
      HistoAxisTitles(hUSS_Y_COPY[ipt], "#it{M}_{ inv}^{ K^{+}#pi^{-}} [GeV/#it{c}^{2}]", "1/#it{N}_{ ev} d#it{N}^{ K#pi}/d#it{M}_{ inv}");
  ...
  Double_t errorMin = 0.75;
  Double_t errorMax = 0.83;
  
  Double_t rhoMinRange = 0.77;
  Double_t rhoMaxRange = 0.85;
  ...
  /* lines->Divide(3,3); */
  for(int fpt=1; fpt<nPtBins+1; fpt++) //nPtBins+1 MUST DELETE +! When i wanna draw one histogram
    {
      /* if(fpt!=5) */
      /*  	continue; */
      ...
      // fill bin
      for(int bin = hUSS_Y_Bkg[nPtBins-fpt]->FindBin(0.85); bin<=hUSS_Y_Bkg[nPtBins-fpt]->FindBin(1.04); bin++)
	hUSS_Y_Bkg[nPtBins-fpt]->SetBinError(bin,0);

      /* Int_t binz=0; */
      /* while(binz<30) */
      /* 	{ */
      /* 	  if(0.777<hUSS_Y_Bkg[nPtBins-fpt]->GetBinCenter(binz)<0.822) */
      /* 	    hUSS_Y_Bkg[nPtBins-fpt]->SetBinError(binz,0); */
      /* 	  else */
      /* 	    break; */
      /* 	  binz++; */
      /* 	} */
      ...
      /* // add */
      /* for(int Bgbin = hUSS_Y_Bkg[nPtBins-fpt]->FindBin(0.75); Bgbin>=hUSS_Y_Bkg[nPtBins-fpt]->FindBin(0.824); Bgbin++) */
      /* 	hUSS_Y_Bkg[nPtBins-fpt]->SetBinError(Bgbin,0); */
      ...
      //=========================================================================
      //| Information : K*0                   | Information : rho0(770) or K*0(800)        | 
      //| gamma(1,3) : 0.0474                 | mass(1) : 0.77526       or 0.841 +-0.00030        | 
      //| mass(2) : 0.89166                   | gamma(2) : 0.1478       or 0.618|  
      //=========================================================================
      fBW_kstar[nPtBins-fpt] = new TF1(Form("BW_kstar_fit_%i",nPtBins-fpt),"[0]*TMath::BreitWigner(x, [1], [2])", kstarMinRange, kstarMaxRange);
      fBW_kstar[nPtBins-fpt]->SetLineColor(8); //Red:2

      double Min = 0.0;
      double Max = 10.0;

      double rhoMassMin = 0.748;
      double rhoMassMax = 0.952;

      double kstarMassMin = 0.89581-0.00019;
      double kstarMassMax = 0.89581+0.00019;
      
      fBW_kstar[nPtBins-fpt]->SetParLimits(0, Min, Max);
      fBW_kstar[nPtBins-fpt]->SetParLimits(1, kstarMassMin, kstarMassMax);
      fBW_kstar[nPtBins-fpt]->FixParameter(2, 0.0474);

      //Information of 
      fBW_rho[nPtBins-fpt] = new TF1(Form("BW_rho_fit_%i",nPtBins-fpt),"[0]*TMath::BreitWigner(x, [1], [2])", rhoMinRange, rhoMaxRange);
      fBW_rho[nPtBins-fpt]->SetLineColor(6);
      
      fBW_rho[nPtBins-fpt]->SetParLimits(0, Min, Max);
      fBW_rho[nPtBins-fpt]->SetParLimits(1, rhoMassMin, rhoMassMax);
      fBW_rho[nPtBins-fpt]->FixParameter(2, 0.618);
      ...
      fBg[nPtBins-fpt] = new TF1(Form("BgFit_%i", nPtBins-fpt), "pol3", Bglow, Bghigh);
      ...
      fBW_kstar[nPtBins-fpt]->SetParameter(0, fFit[nPtBins-fpt]->GetParameter(0));
      fBW_kstar[nPtBins-fpt]->SetParameter(1, fFit[nPtBins-fpt]->GetParameter(1));
      fBW_kstar[nPtBins-fpt]->SetParameter(2, fFit[nPtBins-fpt]->GetParameter(2));
      ...
      HistoAxisTitles(hUSS_Y_COPY[nPtBins-fpt], "#it{M}_{ inv}^{ K^{+}#pi^{-} }[GeV/#it{c}^{2}]",
      ...
      fBW_kstar[nPtBins-fpt]->SetLineColor(1);
      fBW_kstar[nPtBins-fpt]->Draw("SAME");
