...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (int k=1; k<nPtBins+1; k++)
	...
	hUSS_Y_Bgtwo[k]->SetBinError(bin, 0);
      ...
      for (int bin = hUSS_Y_Fit[k]->FindBin(0.85); bin <= hUSS_Y_Fit[k]->FindBin(0.95); bin++)
	...
	fFit[k] = new TF1(Form("Fullfit_%i", k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4) + pol4(9)", ExtraMinRange, BgMaxRange);
	...
	fFit[k]->SetParameter(13, fExtraBgtwo[k]->GetParameter(4));
      ...
      // Third
      fExtraBgtwo_POST[k] = new TF1(Form("kstar_kstarBg_POST_%i",k), "pol4", ExtraMinRange, BgMaxRange);
      
      fExtraBgtwo_POST[k]->SetParameter(0, fFit[k]->GetParameter(9));
      fExtraBgtwo_POST[k]->SetParameter(1, fFit[k]->GetParameter(10));
      fExtraBgtwo_POST[k]->SetParameter(2, fFit[k]->GetParameter(11));
      fExtraBgtwo_POST[k]->SetParameter(3, fFit[k]->GetParameter(12));
      fExtraBgtwo_POST[k]->SetParameter(4, fFit[k]->GetParameter(13));
      ...
      // pol
      /* fExtraBg_POST[k]->SetLineColor(6); */
      /* fExtraBg_POST[k]->SetLineWidth(4); */
      /* fExtraBg_POST[k]->Draw("SAME"); */

      fExtraBgtwo_POST[k]->SetLineColor(6);
