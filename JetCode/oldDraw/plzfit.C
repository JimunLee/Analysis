...
void DrawVoigt(TString Kind, TString DataNum, TString USS, TString LSS)
{
  ...
  TString localpath = "Pid_combined/"+Kind; 
  ...
  //=========================================
  TString SignalName("phi-in-jets/"+USS+DataNum);
  TString BackgroundName("phi-in-jets/"+LSS+DataNum);
  ...
  for(Int_t ipt=0; ipt<nPtBins; ipt++)
	...
	/ hLSS_Y[ipt]->Integral(hLSS_Y[ipt]->FindBin(Nlow), hLSS_Y[ipt]->FindBin(Nhigh));
      ...
      //=========================================
      // plots->cd(ipt +1);
      TCanvas* plots = new TCanvas(Form("plots_%d",ipt), "", 550, 440);
      ...
      hUSS_Y_COPY[ipt]->GetXaxis()->SetTitle("M_{inv}[GeV/#it{c}^{2}]");
      ...
      //int mixedColor = TColor::MixColor(kRed, kWhite, 0.5);
      ...
      TLatex* txtInfoAl = new TLatex(0.41, 0.40, "ALICE Preliminary");
      txtInfoAl->SetNDC(kTRUE); // use NDC
      txtInfoAl->SetTextSize(0.06);
      txtInfoAl->SetTextFont(62);
      txtInfoAl->SetTextColor(1);
      txtInfoAl->Draw("SAME");
      TLatex* txtInfoInc = new TLatex(0.42, 0.35, Kind+" yield");
      txtInfoInc->SetNDC(kTRUE); // use NDC
      txtInfoInc->SetTextSize(0.05);
      txtInfoInc->SetTextFont(62);
      txtInfoInc->SetTextColor(1);
      txtInfoInc->Draw("SAME");
      TLatex* txtInfo = new TLatex(0.42, 0.29, Form("%.1f < p_{T} < %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfo->SetNDC(kTRUE); // NDC coordinates
      txtInfo->SetTextSize(0.05);
      txtInfo->SetTextFont(62);
      txtInfo->SetTextColor(1);
      txtInfo->Draw("SAME");
      TLegend* legend = new TLegend(0.40,0.18,0.65,0.27);
      ...
      // plots->SaveAs("../phi-in-jets/Plot/" + localpath + "/combined_plots.png");
      ...
      TLatex* txtInfoAll = new TLatex(0.50, 0.813, "ALICE Preliminary");
      txtInfoAll->SetNDC(kTRUE); // use NDC
      txtInfoAll->SetTextSize(0.06);
      txtInfoAll->SetTextFont(62);
      txtInfoAll->SetTextColor(1);
      txtInfoAll->Draw("SAME");
      TLatex* txtInfoo = new TLatex(0.51, 0.743, Form("%.1f < p_{T} < %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfoo->SetNDC(kTRUE); // use NDC
      txtInfoo->SetTextSize(0.05);
      txtInfoo->SetTextFont(62);
      txtInfoo->SetTextColor(1);
      txtInfoo->Draw("SAME");
      TLegend* legend_sub = new TLegend(0.51,0.68,0.58,0.70);
      ...
      // substraction->SaveAs("../phi-in-jets/Plot/" + localpath + "/combined_substractions.png");
    }
  ...
  for(int fpt=0; fpt<nPtBins; fpt++) //nPtBins+1 MUST DELETE +! When i wanna draw one histogram
	...
	hUSS_Y_Bkg[fpt]->SetBinError(bin,0);
      ...
      hUSS_Y_COPY[fpt]->GetXaxis()->SetTitle("M_{inv}[GeV/#it{c}^{2}]");
      ...
      TLatex* txtInfoAA = new TLatex(0.46, 0.85, "ALICE Preliminary");
      txtInfoAA->SetNDC(kTRUE); // use NDC
      txtInfoAA->SetTextSize(0.06);
      txtInfoAA->SetTextFont(62);
      txtInfoAA->SetTextColor(1);
      txtInfoAA->Draw("SAME");
      TLatex* txtInfoCC = new TLatex(0.46, 0.79, Kind+" yield");
      txtInfoCC->SetNDC(kTRUE); // use NDC
      txtInfoCC->SetTextSize(0.05);
      txtInfoCC->SetTextFont(62);
      txtInfoCC->SetTextColor(1);
      txtInfoCC->Draw("SAME");
      TLatex* txtInfoBB = new TLatex(0.46, 0.74, Form("%.1f < p_{T} < %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfoBB->SetNDC(kTRUE); // use NDC
      txtInfoBB->SetTextSize(0.05);
      txtInfoBB->SetTextFont(62);
      txtInfoBB->SetTextColor(1);
      txtInfoBB->Draw("SAME");
      TLegend *legendd = new TLegend(0.14,0.17,0.29,0.31);
      legendd->SetFillColor(4000);
      legendd->SetBorderSize(0);
      legendd->SetTextSize(0.05);
      legendd->AddEntry(hUSS_Y_COPY[fpt],"Data", "lpf");
      legendd->AddEntry(fBg[fpt], "Background Fit", "lpf");
      legendd->AddEntry(fFit[fpt], "Signal Fit", "lpf");
      legendd->Draw("SAME");
      
      lines->SaveAs(Form("../phi-in-jets/Plot/"+localpath+"/two_fit_%1.1f_%1.1f.png", ptlow, pthigh));
      //lines->SaveAs("../phi-in-jets/Plot/" + localpath + "/combined_lines.png");
