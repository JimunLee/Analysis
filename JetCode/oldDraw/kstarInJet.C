...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  if (Kind == "Inclusive"){
	...
	if (k==0){
	  ...
	  for (int bin = hUSS_Y_Bg[k]->FindBin(0.85); bin <= hUSS_Y_Bg[k]->FindBin(0.95); bin++)
	    hUSS_Y_Bg[k]->SetBinError(bin, 0);
	  for (int bin = hUSS_Y_ExtraBg[k]->FindBin(0.77); bin <= hUSS_Y_ExtraBg[k]->FindBin(0.84); bin++)
	    hUSS_Y_ExtraBg[k]->SetBinError(bin, 0);
	...
	TFitResultPtr ptr = hUSS_Y_Fit[k]->Fit(fFit[k], "SR0", "", ExtraMinRange, 1.14);
