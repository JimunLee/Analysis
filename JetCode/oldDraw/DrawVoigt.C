...
void DrawVoigt(TString Kind, TString DataNum, TString USS, TString LSS)
{
  ...
  if (Kind == "Outside"){
    ...
    SpecialLatex_plot2 = new TLatex(0.547, 0.228, "#it{p}_{T}^{Jet ch uncorr}> 5 GeV/c");
    ...
    SpecialLatex_fit2 = new TLatex(0.558, 0.727, "#it{p}_{T}^{Jet ch uncorr}> 5 GeV/c");
  ...
  // TString mainfile = "../kstar-in-jets/Result_rootfile/kstar_combined.root";
  ...
  textptb->Draw("SAME");
  TLatex* textptc = new TLatex(0.526, 0.790, "#it{p}_{T}^{ jet ch uncorr} > 5 GeV/#it{c}");
  ...
  for(Int_t ipt=0; ipt<nPtBins; ipt++)
	...
	/ hLSS_Y[ipt]->Integral(hLSS_Y[ipt]->FindBin(Nlow), hLSS_Y[ipt]->FindBin(Nhigh));
      ...
      txtInfoA->Draw("SAME");
      TLatex* txtInfoB = new TLatex(0.55, 0.445, "ALICE #bf{Performance}");
	...
	SpecialLatex_plot2->Draw("SAME");
      ...
      TLatex* txtInfoE = new TLatex(0.543, 0.174, Form("%.1f < #it{p}_{T}^{ K*}< %.1f GeV/#it{c}", ptlow, pthigh));
	...
	SpecialLatex_fit->Draw("SAME");
      ...
      TLatex* txtInfoe = new TLatex(0.55, 0.71, Form("%.1f < #it{p}_{T}^{K*}< %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfoe->SetNDC(kTRUE); // use NDC
      txtInfoe->SetTextSize(0.04);
      txtInfoe->SetTextFont(42);
      txtInfoe->SetTextColor(1);
  ...
  for(int fpt=0; fpt<nPtBins; fpt++) //nPtBins+1 MUST DELETE +! When i wanna draw one histogram
	...
	hUSS_Y_Bkg[fpt]->SetBinError(bin,0);
      ...
      TLatex* txtInfoEE = new TLatex(0.56, 0.673, Form("%.1f < #it{p }_{T}^{K*}< %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfoEE->SetNDC(kTRUE); // use NDC
      txtInfoEE->SetTextSize(0.04);
      txtInfoEE->SetTextFont(42);
      txtInfoEE->SetTextColor(1);
      txtInfoEE->Draw("SAME");
      TLatex* txtInfoFF = new TLatex(0.56, 0.621, chi2NdfText.c_str());
      ...
      legendd->AddEntry(hUSS_Y_COPY[fpt],"Data. #it{M }_{inv}^{K^{+}#pi^{-}}", "lpf");
