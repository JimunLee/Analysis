...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (Int_t j=0; j<nPtBins; j++)
	...
	if (j==0){
	  ...
	  TLatex* txtInfoB = DrawTLatex(0.185, 0.375, "ALICE #bf{WIP}", 0.05, 62, 1);
	  ...
	  TLatex* txtInfoB = DrawTLatex(0.641, 0.929, "ALICE #bf{WIP}", 0.05, 62, 1);
	...
	if (j==0){
	  ...
	  TLegend* legend = new TLegend(0.60,0.16,0.90,0.31); //LEFT x, y, Right x, y
	  legend->SetFillColor(4000);
	  legend->SetBorderSize(0);
	  legend->SetTextSize(0.05);
	  legend->AddEntry(hUSS_Y[j], "K^{+}#pi^{-}", "lpf");
	...
	if (j==0){
	  ...
	  TLatex* txtInfoB = DrawTLatex(0.185, 0.929, "ALICE #bf{WIP}", 0.05, 62, 1);
	  ...
	  TLatex* txtInfoE = DrawTLatex(0.185, 0.665, "#it{p}_{T}^{ Jet ch uncorr} > 8 GeV/c", 0.04, 42, 1);
	  TLatex* txtInfoF = DrawTLatex(0.185, 0.723, Form("%.1f < #it{p}_{T}^{ K*}< %.1f GeV/#it{c}", ptlow, pthigh), 0.04, 42, 1);
	}
	else{
  ...
  if (Kind == "Inclusive"){
	...
	// Function
	// ==============
	fBg[k] = new TF1(Form("kstar_Bg_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x)", ExtraMinRange, BgMaxRange);
	fExtraBg[k] = new TF1(Form("kstar_ExtraBg_%i",k), "pol4(4)", ExtraMinRange, BgMaxRange);
	...
	fFit_Tmp2[k] = new TF1(Form("FullfitTmp_%i", k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4) + [9]*BreitWignerRelativistic(x, [10], [11])", ExtraMinRange, BgMaxRange);
	...
	if(k==0){
	  ...
	  //Temp1 : 3.5 ~ 5.0
	  //===================
	  for (int bin = hUSS_Y_BgTmp[k]->FindBin(0.82); bin <= hUSS_Y_BgTmp[k]->FindBin(0.98); bin++)
	    hUSS_Y_BgTmp[k]->SetBinError(bin, 0);
	  for (int bin = hUSS_Y_kstarSigTmp[k]->FindBin(0.79); bin <= hUSS_Y_kstarSigTmp[k]->FindBin(0.825); bin++)
	  ...
	  //Temp2 : 1.0 ~ 1.5
	  //===================
	  for (int bin = hUSS_Y_BgTmp2[k]->FindBin(0.83); bin <= hUSS_Y_BgTmp2[k]->FindBin(0.99); bin++)
	    hUSS_Y_BgTmp2[k]->SetBinError(bin, 0);
	  for (int bin = hUSS_Y_kstarSigTmp2[k]->FindBin(0.75); bin <= hUSS_Y_kstarSigTmp2[k]->FindBin(0.79); bin++)
	...
	double yMin = hYield->GetMinimum()*5;
	double yMax = hYield->GetMaximum()*1.4;
  ...
  // Jet
	...
	if (k==0){
	  ...
	  for (int bin = hUSS_Y_BgTmp[k]->FindBin(0.84); bin <= hUSS_Y_BgTmp[k]->FindBin(0.98); bin++)
	    hUSS_Y_BgTmp[k]->SetBinError(bin, 0);
	  for (int bin = hUSS_Y_kstarSigTmp[k]->FindBin(0.75); bin <= hUSS_Y_kstarSigTmp[k]->FindBin(0.84); bin++)
	    hUSS_Y_kstarSigTmp[k]->SetBinError(bin, 0);
	...
	double yMin = hYield->GetMinimum()*1.1;
