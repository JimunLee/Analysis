...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  TF1* fBg [nPtBins];
  TF1* fFit [nPtBins];
  TF1* fcomplete_kstar [nPtBins];
  TF1* fcomplete_omega [nPtBins];
  TF1* fcomplete_Bg [nPtBins];
  ...
  for (int k=0; k<nPtBins; k++)
    {
      ...
      /* // add */
      /* for(int bin = hUSS_Y_Fit[k]->FindBin(0.75); bin<=hUSS_Y_Fit[k]->FindBin(0.827); bin++) */
      /* 	hUSS_Y_Fit[k]->SetBinError(bin,0); */
      
      /* // fill bin */
      /* int startBin = hUSS_Y_Fit[k]->FindBin(0.87); */
      /* int endBin = hUSS_Y_Fit[k]->FindBin(0.79); */
      /* double refValue = hUSS_Y_Fit[k]->GetBinContent(startBin); */
      
      /* for(int bin = startBin -1; bin >= endBin; --bin){ */
      /* 	double currentValue = hUSS_Y_Fit[k]->GetBinContent(bin); */
      ...
      // fitting kstar
      //BW
      fBW_kstar[k] = new TF1(Form("kstar_bw_%i",k), "[0]*BreitWigner(x, [1], [2])", kstarMinRange, kstarMaxRange);

      fBW_kstar[k]->SetParLimits(0, Min, Max);
      fBW_kstar[k]->SetParLimits(1, kstarMassMin, kstarMassMax);
      fBW_kstar[k]->FixParameter(2, 0.0474);
     
      hUSS_Y_Fit[k]->Fit(fBW_kstar[k], "0", "", kstarMinRange, kstarMaxRange);

      //omega
      fBW_omega[k] = new TF1(Form("omega_bw_%i", k), "[0]*BreitWignerRelativistic(x, [1], [2])", omegaMinRange, omegaMaxRange);

      fBW_omega[k]->SetParLimits(0, Min, 0.000001);
      fBW_omega[k]->SetParLimits(1, omegaMassMin, omegaMassMax);
      fBW_omega[k]->FixParameter(2, 0.00849);

      hUSS_Y_Fit[k]->Fit(fBW_omega[k], "0", "", omegaMassMin, omegaMassMax);
    
      // =====================
      // fitting background
      fBg[k] = new TF1(Form("BgFit_%i", k), "pol3(0)", Bglow, Bghigh);

      fBg[k]->SetParLimits(0, 0.000001, 0.001);
      fBg[k]->SetParLimits(1, -1, 1);
      fBg[k]->SetParameter(2, 2);
      fBg[k]->SetParameter(3, 3);
      
      hUSS_Y_Fit[k]->Fit(fBg[k], "0", "", Bglow, Bghigh);
      
      // ===============
      // fitting entire
      fFit[k] = new TF1(Form("Fullfit_%i", k), "[0]*BreitWigner(x, [1], [2]) + [3]*BreitWignerRelativistic(x, [4], [5]) + pol3(6)", Bglow, Bghigh);
 
      fFit[k]->SetParameter(0, fBW_kstar[k]->GetParameter(0));
      fFit[k]->SetParameter(1, fBW_kstar[k]->GetParameter(1));
      fFit[k]->SetParameter(2, fBW_kstar[k]->GetParameter(2));

      fFit[k]->SetParameter(3, fBW_omega[k]->GetParameter(0));
      fFit[k]->SetParameter(4, fBW_omega[k]->GetParameter(1));
      fFit[k]->SetParameter(5, fBW_omega[k]->GetParameter(2));
      
      fFit[k]->SetParameter(6, fBg[k]->GetParameter(0));
      fFit[k]->SetParameter(7, fBg[k]->GetParameter(1));
      fFit[k]->SetParameter(8, fBg[k]->GetParameter(2));
      fFit[k]->SetParameter(9, fBg[k]->GetParameter(3));
      ...
      fcomplete_kstar[k] = new TF1(Form("kstar_bw_%i",k), "[0]*BreitWigner(x, [1], [2])", kstarMinRange, kstarMaxRange);
      fcomplete_omega[k] = new TF1(Form("omega_bw_%i", k), "[0]*BreitWignerRelativistic(x, [1], [2])", omegaMinRange, omegaMaxRange);
      fcomplete_Bg[k] = new TF1(Form("BgFit_%i", k), "pol3(0)", Bglow, Bghigh);
      
      fcomplete_kstar[k]->SetParameter(0, fFit[k]->GetParameter(0));
      fcomplete_kstar[k]->SetParameter(1, fFit[k]->GetParameter(1));
      fcomplete_kstar[k]->SetParameter(2, fFit[k]->GetParameter(2));

      fcomplete_omega[k]->SetParameter(0, fFit[k]->GetParameter(3));
      fcomplete_omega[k]->SetParameter(1, fFit[k]->GetParameter(4));
      fcomplete_omega[k]->SetParameter(2, fFit[k]->GetParameter(5));
            
      fcomplete_Bg[k]->SetParameter(0, fFit[k]->GetParameter(6));
      fcomplete_Bg[k]->SetParameter(1, fFit[k]->GetParameter(7));
      fcomplete_Bg[k]->SetParameter(2, fFit[k]->GetParameter(8));
      fcomplete_Bg[k]->SetParameter(3, fFit[k]->GetParameter(9));
      ...
      fcomplete_kstar[k]->SetLineColor(6);
      fcomplete_kstar[k]->Draw("SAME");
      
      fcomplete_Bg[k]->SetLineColor(4);
      fcomplete_Bg[k]->SetLineStyle(2);
      fcomplete_Bg[k]->Draw("SAME");
      
      fcomplete_omega[k]->SetLineColor(8);
      fcomplete_omega[k]->Draw("SAME");
