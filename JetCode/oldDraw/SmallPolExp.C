...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (Int_t i=0; i<nPtBins; i++)
    {
      ...
      hUSS_Y_kstarBg[i] = new TH1D(Form("hUSS_Y_kstarBg_%i", i), Form("hUSS_Y_kstarBg_%i", i), nBins, imlow, imtop);
  ...
  TF1* fkstarBg_POST [nPtBins];
  TF1* fkstarANDBg_POST [nPtBins];
  ...
  for (int k=1; k<nPtBins+1; k++)
	...
	continue;
      ...
      hUSS_Y_kstarBg[k]->Add(hUSS_Y_SUB[k]);
      hUSS_Y_kstarSig[k]->Add(hUSS_Y_SUB[k]);
      hUSS_Y_Bg[k]->Add(hUSS_Y_SUB[k]);
      
      hUSS_Y_Fit[k]->Add(hUSS_Y_SUB[k]);

      for (int bin = hUSS_Y_Bg[k]->FindBin(0.825); bin <= hUSS_Y_Bg[k]->FindBin(1.05); bin++)
	hUSS_Y_Bg[k]->SetBinError(bin, 0);

      for (int bin = hUSS_Y_Fit[k]->FindBin(0.825); bin <= hUSS_Y_Fit[k]->FindBin(0.98); bin++)
	hUSS_Y_Fit[k]->SetBinError(bin, 0);
      ...
      // 1. Extra fit
      fBg[k] = new TF1(Form("kstar_ExtraBg_%i",k), "exp([0] + [1]*x + [2]*x*x)", ExtraMinRange, BgMaxRange);
      hUSS_Y_Bg[k]->Fit(fBg[k], "0", "", ExtraMinRange, BgMaxRange);
      
      // 2. Background fit
      fExtraBg[k] = new TF1(Form("kstar_bg_%i",k), "pol3", ExtraMinRange, BgMaxRange);
      hUSS_Y_Bg[k]->Fit(fExtraBg[k], "0", "", ExtraMinRange, BgMaxRange);

      // 3. kstar fit
      /* fkstar[k] = new TF1(Form("kstar_sig_%i",k), "[0]*BreitWignerRelativistic(x, [1], [2])", kstarMinRange, kstarMaxRange); */
      /* fkstar[k]->SetParameter(1, kstarMass); */
      /* fkstar[k]->SetParameter(2, 0.0474); */
      /* hUSS_Y_Fit[k]->Fit(fkstar[k], "0", "", kstarMinRange, kstarMaxRange); */
      // huss_Y_Fit[k]->Add(hUSS_Y_kstarSig[k]);
      
  
      // ===============
      // fitting entire
      if(k==1){
	//	fFit[k] = new TF1(Form("Fullfit_%i", k), "[0]*BreitWignerRelativistic(x, [1], [2]) + pol4(4)", ExtraMinRange, kstarMaxRange);
	fFit[k] = new TF1(Form("Fullfit_%i", k), "exp([0] + [1]*x + [2]*x*x) + pol3(3)", ExtraMinRange, BgMaxRange);
	...
	fFit[k]->SetParameter(3, fExtraBg[k]->GetParameter(0));
	fFit[k]->SetParameter(4, fExtraBg[k]->GetParameter(1));
	fFit[k]->SetParameter(5, fExtraBg[k]->GetParameter(2));
	fFit[k]->SetParameter(6, fExtraBg[k]->GetParameter(3));
	
	/* fFit[k]->SetParameter(7, fkstar[k]->GetParameter(0)); */
	/* fFit[k]->SetParameter(8, fkstar[k]->GetParameter(1)); */
	/* fFit[k]->SetParameter(9, fkstar[k]->GetParameter(2)); */
      ...
      // First
      fBg_POST[k] = new TF1(Form("kstar_ExtraBg_POST_%i",k), "exp([0] + [1]*x + [2]*x*x)", ExtraMinRange, BgMaxRange);
      ...
      /* fkstar_POST[k]->SetParameter(0, fFit[k]->GetParameter(7)); */
      /* fkstar_POST[k]->SetParameter(1, fFit[k]->GetParameter(8)); */
      /* fkstar_POST[k]->SetParameter(2, fFit[k]->GetParameter(9)); */
      
      // Second
      fExtraBg_POST[k] = new TF1(Form("kstar_kstarBg_POST_%i",k), "pol3", ExtraMinRange, BgMaxRange);
      
      fExtraBg_POST[k]->SetParameter(0, fFit[k]->GetParameter(3));
      fExtraBg_POST[k]->SetParameter(1, fFit[k]->GetParameter(4));
      fExtraBg_POST[k]->SetParameter(2, fFit[k]->GetParameter(5));
      fExtraBg_POST[k]->SetParameter(3, fFit[k]->GetParameter(6));
      ...
      /* fkstarBg[k]->SetLineColor(4); */
      /* fkstarBg[k]->SetLineWidth(4); */
      /* fkstarBg[k]->Draw("SAME"); */

      /* fExtraBg_POST[k]->SetLineColor(7); */
      /* fExtraBg_POST[k]->SetLineWidth(4); */
      /* fExtraBg_POST[k]->Draw("SAME"); */
      
      /* fExtraBg[k]->SetLineColor(7); */
      /* fExtraBg[k]->SetLineWidth(4); */
      /* fExtraBg[k]->Draw("SAME"); */
      
      /* fkstar[k]->SetLineColor(6); */
      /* fkstar[k]->SetLineWidth(4); */
      /* fkstar[k]->Draw("SAME"); */
      ...
      /* fBg[k]->SetLineColor(4); */
      /* fBg[k]->SetLineWidth(4); */
      /* fBg[k]->Draw("SAME"); */
