...
void DrawVoigt(TString Kind, TString DataNum, TString USS, TString LSS)
{
  ...
  TString localpath = "extra/"+Kind; 

  //=========================================
  // Data file inputs
  //=========================================

  TString mainfile = "../phi-in-jets/Result_rootfile/output_totcombined.root";
  ...
  for(Int_t ipt=0; ipt<nPtBins; ipt++)
	...
	/ hLSS_Y[ipt]->Integral(hLSS_Y[ipt]->FindBin(Nlow), hLSS_Y[ipt]->FindBin(Nhigh));
      ...
      TLatex* txtInfoAl = new TLatex(0.50, 0.42, "ALICE");
      txtInfoAl->SetNDC(kTRUE); // use NDC
      txtInfoAl->SetTextSize(0.07);
      txtInfoAl->SetTextFont(62);
      txtInfoAl->SetTextColor(1);
      txtInfoAl->Draw("SAME");
      TLatex* txtInfo = new TLatex(0.50, 0.35, Form("%.1f < p_{T} < %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfo->SetNDC(kTRUE); // NDC coordinates
      txtInfo->SetTextSize(0.06);
      txtInfo->SetTextFont(62);
      txtInfo->SetTextColor(1);
      txtInfo->Draw("SAME");
      TLegend* legend = new TLegend(0.50,0.19,0.81,0.31);
      ...
      TLatex* txtInfoAll = new TLatex(0.60, 0.83, "ALICE");
      txtInfoAll->SetNDC(kTRUE); // use NDC
      txtInfoAll->SetTextSize(0.05);
      txtInfoAll->SetTextFont(62);
      txtInfoAll->SetTextColor(1);
      txtInfoAll->Draw("SAME");
      TLatex* txtInfoo = new TLatex(0.60, 0.77, Form("%.1f < p_{T} < %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfoo->SetNDC(kTRUE); // use NDC
      txtInfoo->SetTextSize(0.04);
      txtInfoo->SetTextFont(62);
      txtInfoo->SetTextColor(1);
      txtInfoo->Draw("SAME");
      TLegend* legend_sub = new TLegend(0.79,0.70,0.59,0.75);
  ...
  for(int fpt=0; fpt<nPtBins; fpt++) //nPtBins+1 MUST DELETE +! When i wanna draw one histogram
	...
	hUSS_Y_Bkg[fpt]->SetBinError(bin,0);
      ...
      TLatex* txtInfoAA = new TLatex(0.60, 0.83, "ALICE");
      txtInfoAA->SetNDC(kTRUE); // use NDC
      txtInfoAA->SetTextSize(0.05);
      txtInfoAA->SetTextFont(62);
      txtInfoAA->SetTextColor(1);
      txtInfoAA->Draw("SAME");
      TLatex* txtInfoBB = new TLatex(0.60, 0.77, Form("%.1f < pT < %.1f GeV/#it{c}", ptlow, pthigh));
      txtInfoBB->SetNDC(kTRUE); // use NDC
      txtInfoBB->SetTextSize(0.04);
      txtInfoBB->SetTextFont(62);
      txtInfoBB->SetTextColor(1);
      txtInfoBB->Draw("SAME");
      TLegend *legendd = new TLegend(0.79,0.60,0.59,0.75);
