...
/* Double_t threshold = 0.692;//Sum of kaon energy + pion energy */
double PhaseSpaceFactor(double x, double T, double pT) {
  return ( x / ( sqrt( pow(x,2) + pow(pT,2) ) ) ) * exp( -sqrt( pow(x,2) + pow(pT,2) )/T );
}
double breitWigner(double x, double Amp, double M0, double gamma) {
  double Pi = 3.141592;
  double bw = Amp*gamma;
  bw /= (2*Pi*pow(x-M0, 2) + M0*M0/4);
  return bw;
}

double breitWigner_Kstar(double *x, double* par) {
  return breitWigner(x[0], par[1], par[0], par[2]);
}
...
double sfit(double *x, double *par) {
  ...
  /* double bgConst = par[6]; */
  /* double bgSloop = par[7]; */
  /* double bg2 = par[8]; */
  /* double bg3 = par[9]; */
  ...
  double breitWigner_Kstar = breitWigner(x[0], kstarA, kstarM, kstarG);
  double breitWigner_rho = breitWigner(x[0], rhoA, rhoM, rhoG);
  /* double Bgfunc = background(x[0], bgConst, bgSloop, bg2, bg3); */

  return (breitWigner_Kstar + breitWigner_rho)*PhaseSpaceFactor(x[0], T, pT);
}
...
void DrawVoigt(TString Kind, TString USS, TString LSS)
{
  ...
  // K*0  signal range
  Double_t Siglow = 0.76;
  Double_t Sighigh = 1.04;
  // maybe omega signal range
  Double_t extSiglow = 0.76;
  Double_t extSighigh = 0.84;
  Double_t Bglow = 0.75;
  ...
  for(int fpt=0; fpt<nPtBins; fpt++) //nPtBins+1 MUST DELETE +! When i wanna draw one histogram
	...
	hUSS_Y_Bkg[fpt]->SetBinError(bin,0);
      ...
      //| gamma(1,3) : 0.0474                 | mass(1) : 0.77526               | mass(1) : 0.78265         |
      ...
      sFit[fpt] = new TF1(Form("breitWignerFit_%i",fpt), sfit, Bglow, Bghigh);
      sFit[fpt]->SetLineColor(8);

      hUSS_Y_Peak[fpt]->Fit(sFit[fpt], "0", "", Bglow, Bghigh);
      ...
      fFit[fpt] = new TF1(Form("FullFit_%i",fpt), "sfit + pol3(6)", Bglow, Bghigh);
      fFit[fpt]->SetLineColor(2);
      fFit[fpt]->FixParameter(10, 0.160);
      fFit[fpt]->FixParameter(11, (ptlow+pthigh)/2.);

      fFit[fpt]->SetParLimits(0, 0.85, 0.95);
      fFit[fpt]->SetParLimits(1, 1.0*2.0, 1e6*2.5);
      fFit[fpt]->SetParameter(2, 0.0474);
      
      fFit[fpt]->SetParameter(3, 0.77526);
      fFit[fpt]->SetParLimits(4, 1.0*2.0, 1e6*2.0);
      fFit[fpt]->SetParameter(5, 0.1478);
      ...
      ////////////////////////////////////////////////////////////////////////////////////////////////////////
      /* fBreit[fpt] = new TF1(Form("BwFit_%i",fpt),"[0]*TMath::BreitWigner(x, [1], [2])", Siglow, Sighigh); */
      /* fBreit[fpt]->SetLineColor(2); //Red:2 */
      
      /* fBreit[fpt]->SetParameter(0,1.0/nEvents); */
      /* fBreit[fpt]->SetParameter(1,0.89176); */
      /* fBreit[fpt]->SetParameter(2,0.0474); */

      /* //Information of omega(782) */
      /* fgaus[fpt] = new TF1(Form("gaFit_%i",fpt),"[3]*TMath::BreitWigner(x, [4], [5])", extSiglow, extSighigh); */
      /* fgaus[fpt]->SetLineColor(2); //Red:2 */
      
      /* fgaus[fpt]->SetParameter(3,1.0/nEvents); */
      /* fgaus[fpt]->SetParameter(4,0.78265); */
      /* fgaus[fpt]->SetParameter(5,0.0849); */
      ...
      /* hUSS_Y_Peak[fpt]->Fit(fgaus[fpt], "0", "", extSiglow, extSighigh); */
      /* hUSS_Y_Peak[fpt]->Fit(fgaus[fpt], "0", "", extSiglow, extSighigh); */
      /* hUSS_Y_Peak[fpt]->Fit(fgaus[fpt], "0", "", extSiglow, extSighigh); */
      /* hUSS_Y_Peak[fpt]->Fit(fgaus[fpt], "0", "", extSiglow, extSighigh); */
      
      /* // fBreitCopy[fpt] = new TF1(Form("BwFit_Copy_%i",fpt),"[0]*TMath:::BreitWigner(x, [1], [2])", Siglow, Sighigh); */
      ...
      /* //========================================= */
      /* fFit[fpt] = new TF1(Form("FullFit_%i",fpt), "[0]*TMath::BreitWigner(x, [1], [2]) + [3]*TMath::BreitWigner(x, [4], [5]) + pol3(6)", Bglow, Bghigh); */
      /* fFit[fpt]->SetLineColor(2); */
      ...
      /* fFit[fpt]->SetParameter(3, fgaus[fpt]->GetParameter(0)); */
      /* fFit[fpt]->SetParameter(4, fgaus[fpt]->GetParameter(1)); */
      /* fFit[fpt]->SetParameter(5, fgaus[fpt]->GetParameter(2)); */
      
      /* //========================================= */
      /* // Combine Peak + Background : Extra */
      /* //========================================= */
      ...
      /* fFit[fpt]->SetParameter(3, fgaus[fpt]->GetParameter(3)); */
      /* fFit[fpt]->SetParameter(4, fgaus[fpt]->GetParameter(4)); */
      /* fFit[fpt]->SetParameter(5, fgaus[fpt]->GetParameter(5)); */
      ...
      /* /\* fgaus[fpt]->SetParameters(fFit[fpt]->GetParameters()); *\/ */
