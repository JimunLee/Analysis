...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (int k=1; k<nPtBins+1; k++)
	...
	continue;
      ...
      for (int bin = hUSS_Y_Bg[k]->FindBin(0.84); bin <= hUSS_Y_Bg[k]->FindBin(0.95); bin++)
	...
	hUSS_Y_kstarSig[k]->SetBinError(bin, 0);
      ...
      // fitting entire
      if(k==1){
	fFit[k] = new TF1(Form("Fullfit_%i", k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol3(4) + [8]*BreitWignerRelativistic(x, [9], [10])", ExtraMinRange, BgMaxRange);
	...
	fFit[k]->SetParameter(8, fkstar[k]->GetParameter(0));
	fFit[k]->SetParameter(9, fkstar[k]->GetParameter(1));
	fFit[k]->SetParameter(10, fkstar[k]->GetParameter(2));
      ...
      // First
      fBg_POST[k] = new TF1(Form("kstar_ExtraBg_POST_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol3(4)", ExtraMinRange, BgMaxRange);
      ...
      fkstar_POST[k]->SetParameter(0, fFit[k]->GetParameter(8));
      fkstar_POST[k]->SetParameter(1, fFit[k]->GetParameter(9));
      fkstar_POST[k]->SetParameter(2, fFit[k]->GetParameter(10));
