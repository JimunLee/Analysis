...
void DrawVoigt(TString Kind, TString DataNum)
{
  ...
  for(int fpt=0; fpt<nPtBins; fpt++)
	...
	hUSS_Y_Bkg[fpt]->SetBinError(bin,0);
      ...
      /* fBreit_only[fpt]->SetParameter(0, fBreit[fpt]->GetParameter(4)); */
      /* fBreit_only[fpt]->SetParameter(1, fBreit[fpt]->GetParameter(5)); */
      /* fBreit_only[fpt]->SetParameter(2, fBreit[fpt]->GetParameter(6)); */

      //=========================================
      // Fitting Background Function + condition
      //=========================================
	...
	//}
      ...
      /* fBreit_only[fpt]->SetLineColor(5); //5 : pink */
      /* fBreit_only[fpt]->Draw("SAME"); */
      /* fBg_only[fpt]->SetLineColor(6); // 6: cyan */
      ...
      while(binz<25)
	{
	  if(hUSS_Y[fpt]->GetBinCenter(binz)<0.749735)
	    hUSS_Y[fpt]->SetBinError(binz,0);
	  else
	    break;
	  binz++;
	}
      cout<<hUSS_Y[fpt]->GetBinCenter(hUSS_Y[fpt]->FindBin(Sighigh))<<endl;
	...
	binpiece=0;
      ...
      cout<<"Bins :"<<xBins[fpt]<<" - " << xBins[fpt+1]<<endl;
      cout<<"fpt : "<<fpt<<endl;
      cout<<"Ndf :"<<fFit[fpt]->GetNDF()<<endl;
