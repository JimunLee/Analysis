...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  TH1* hUSS_Y_rho [nPtBins];
  TH1* hUSS_Y_omega [nPtBins];
  ...
  for (Int_t i=0; i<nPtBins; i++)
    {
      ...
      hUSS_Y_rho[i] = new TH1D(Form("hUSS_Y_rho_%i", i), Form("hUSS_Y_rho_%i", i), nBins, imlow, imtop);
      hUSS_Y_omega[i] = new TH1D(Form("hUSS_Y_omega_%i", i), Form("hUSS_Y_omega_%i", i), nBins, imlow, imtop);
  ...
  double f0MinRange = 0.75;
  double f0MaxRange = 1.15;
  ...
  for (int k=1; k<nPtBins+1; k++)
    {
      ...
      hUSS_Y_rho[nPtBins-k]->Add(hUSS_Y_SUB[nPtBins-k]);
      hUSS_Y_omega[nPtBins-k]->Add(hUSS_Y_SUB[nPtBins-k]);
      hUSS_Y_Bkg[nPtBins-k]->Add(hUSS_Y_SUB[nPtBins-k]);

      for (int bin = hUSS_Y_Bkg[nPtBins-k]->FindBin(0.83); bin <= hUSS_Y_Bkg[nPtBins-k]->FindBin(0.97); bin++){
	hUSS_Y_rho[nPtBins-k]->SetBinError(bin, 0);
	hUSS_Y_omega[nPtBins-k]->SetBinError(bin, 0);
      }

      if (k == nPtBins-1)
	{
        for (int bin = hUSS_Y_Bkg[nPtBins-k]->FindBin(0.76); bin <= hUSS_Y_Bkg[nPtBins-k]->FindBin(0.819); bin++)
	  hUSS_Y_Bkg[nPtBins-k]->SetBinError(bin, 0);
        for (int bin = hUSS_Y_Bkg[nPtBins-k]->FindBin(0.83); bin <= hUSS_Y_Bkg[nPtBins-k]->FindBin(0.962); bin++)
	  hUSS_Y_Bkg[nPtBins-k]->SetBinError(bin, 0);
	}
      else
	{
      ...
      /* hUSS_Y_rho[nPtBins-k]->Add(hUSS_Y_Bkg[nPtBins-k]); */
      /* hUSS_Y_omega[nPtBins-k]->Add(hUSS_Y_Bkg[nPtBins-k]); */
      ...
      /* fBW_kstar[nPtBins-k]->FixParameter(1, kstarMass); */
      ...
      /* fBW_omega[nPtBins-k]->FixParameter(1, omegaMass); */
      /* fBW_omega[nPtBins-k]->FixParameter(2, 0.00849); */

      /* hUSS_Y_omega[nPtBins-k]->Fit(fBW_omega[nPtBins-k], "0", "", omegaMinRange, omegaMaxRange); */
      /* hUSS_Y_Fit[nPtBins-k]->Add(hUSS_Y_omega[nPtBins-k]); */

      //rho
      fBW_rho[nPtBins-k] = new TF1(Form("rho_bw_%i", nPtBins-k), "[3]*BreitWignerRelativistic(x, [4], [5])", rhoMinRange, rhoMaxRange);

      fBW_rho[nPtBins-k]->SetParLimits(3, Min, Max);
      fBW_rho[nPtBins-k]->FixParameter(4, rhoMass);
      fBW_rho[nPtBins-k]->FixParameter(5, 0.1478);

      hUSS_Y_rho[nPtBins-k]->Fit(fBW_rho[nPtBins-k], "0", "", rhoMinRange, rhoMaxRange);
      hUSS_Y_Bkg[nPtBins-k]->Add(hUSS_Y_rho[nPtBins-k]);

      // =====================
      // fitting background
      fBg[nPtBins-k] = new TF1(Form("BgFit_%i", nPtBins-k), "pol3(0)", Bglow, Bghigh);

      fBg[nPtBins-k]->GetParameter(0);
      fBg[nPtBins-k]->GetParameter(1);
      fBg[nPtBins-k]->GetParameter(2);
      fBg[nPtBins-k]->GetParameter(3);
      
      hUSS_Y_Bkg[nPtBins-k]->Fit(fBg[nPtBins-k], "0", "", Bglow, Bghigh);
      //   hUSS_Y_Fit[nPtBins-k]->Add(hUSS_Y_Bkg[nPtBins-k]);
      
      // ===============
      // fitting entire
      fFit[nPtBins-k] = new TF1(Form("Fullfit_%i", nPtBins-k), "pol3(6) + [3]*BreitWignerRelativistic(x, [4], [5])", Bglow, Bghigh);
      
      /* fFit[nPtBins-k] = new TF1(Form("Fullfit_%i", nPtBins-k), "[0]*BreitWigner(x, [1], [2]) + pol3(6) + [3]*BreitWignerRelativistic(x, [4], [5]) +[10]*BreitWignerRelativistic(x, [11], [12])", Bglow, Bghigh); */
      ...
      fFit[nPtBins-k]->SetParLimits(3, 0, fBW_rho[nPtBins-k]->GetParameter(3));
      fFit[nPtBins-k]->SetParameter(4, fBW_rho[nPtBins-k]->GetParameter(4));
      fFit[nPtBins-k]->SetParameter(5, fBW_rho[nPtBins-k]->GetParameter(5));

      fFit[nPtBins-k]->SetParameter(6, fBg[nPtBins-k]->GetParameter(0));
      fFit[nPtBins-k]->SetParameter(7, fBg[nPtBins-k]->GetParameter(1));
      fFit[nPtBins-k]->SetParameter(8, fBg[nPtBins-k]->GetParameter(2));
      fFit[nPtBins-k]->SetParameter(9, fBg[nPtBins-k]->GetParameter(3));

      /* fFit[nPtBins-k]->SetParLimits(10, 0, fBW_omega[nPtBins-k]->GetParameter(0)); */
      /* fFit[nPtBins-k]->SetParameter(11, fBW_omega[nPtBins-k]->GetParameter(1)); */
      /* fFit[nPtBins-k]->SetParameter(12, fBW_omega[nPtBins-k]->GetParameter(2)); */

      /* hUSS_Y_Fit[nPtBins-k]->GetXaxis()->SetRangeUser(0.75, 1.15); */
      /* hUSS_Y_Fit[nPtBins-k]->Fit(fFit[nPtBins-k], "SR0", "", Bglow, Bghigh); */

      hUSS_Y_Bkg[nPtBins-k]->GetXaxis()->SetRangeUser(0.75, 1.15);
      hUSS_Y_Bkg[nPtBins-k]->Fit(fFit[nPtBins-k], "SR0", "", Bglow, Bghigh);
      ...
      fBW_rho[nPtBins-k]->SetParameter(3, fFit[nPtBins-k]->GetParameter(3));
      fBW_rho[nPtBins-k]->SetParameter(4, fFit[nPtBins-k]->GetParameter(4));
      fBW_rho[nPtBins-k]->SetParameter(5, fFit[nPtBins-k]->GetParameter(5));
      
      fBg[nPtBins-k]->SetParameter(0, fFit[nPtBins-k]->GetParameter(6));
      fBg[nPtBins-k]->SetParameter(1, fFit[nPtBins-k]->GetParameter(7));
      fBg[nPtBins-k]->SetParameter(2, fFit[nPtBins-k]->GetParameter(8));
      fBg[nPtBins-k]->SetParameter(3, fFit[nPtBins-k]->GetParameter(9));
      
      /* fBW_omega[nPtBins-k]->SetParameter(0, fFit[nPtBins-k]->GetParameter(10)); */
      /* fBW_omega[nPtBins-k]->SetParameter(1, fFit[nPtBins-k]->GetParameter(11)); */
      /* fBW_omega[nPtBins-k]->SetParameter(2, fFit[nPtBins-k]->GetParameter(12)); */

      TFitResultPtr ptr = hUSS_Y_Bkg[nPtBins-k]->Fit(fFit[nPtBins-k], "SR0", "", Bglow, Bghigh);
      ...
      hUSS_Y_Bkg[nPtBins-k]->SetTitle(Form(""));
      
      hUSS_Y_Bkg[nPtBins-k]->SetLineColor(1);
      hUSS_Y_Bkg[nPtBins-k]->Draw();
