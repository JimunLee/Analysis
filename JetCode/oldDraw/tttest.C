...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  if (Kind == "Inclusive"){
    Double_t inclusive_xBins[nPtBins+1] = {0.5, 1.0, 1.5, 2.0, 2.5, 3.5, 5.0, 8.0};
    copy(begin(inclusive_xBins), end(inclusive_xBins), xBins);

    SpecialLatex_fit = DrawTLatex(0.557, 0.79, "Min. Bias", 0.04, 42, 1);
  }
  else if (Kind == "Outside"){
    Double_t outside_xBins[nPtBins+1] = {0.5, 1.0, 1.5, 2.0, 2.5, 3.5, 5.0, 8.0};
  ...
  TLatex* textpTa = DrawTLatex(0.526, 0.915, "ALICE #bf{Performance}", 0.05, 62, 1);
  ...
  FJc->SetLogy();
  FJc->SaveAs("../kstar-in-jets/Plot/"+widelocal+"FastJetpt.pdf", "RECREATE");
  ...
  for (Int_t j=0; j<nPtBins; j++)
    {
      ...
      HistoAxisTitles(hUSS_Y[j], "#it{M}_{ inv}^{ K^{+}#pi^{-}} [GeV/#it{c}^{2}]", "d#it{N}^{ K#pi}/d#it{M}_{ inv}", 0.06, 62, 1.0, 0.06, 62, 1.2);
      ...
      hUSS_Y[j]->GetXaxis()->SetRangeUser(0.75, 1.24);
      hUSS_Y[j]->Draw();
      ...
      hLSS_Y[j]->GetXaxis()->SetRangeUser(0.75, 1.24);
      hLSS_Y[j]->Draw("SAME");

      TH1D* hShade = (TH1D*)hLSS_Y[j]->Clone(Form("hShade_%d", j));
	...
	if (bin < binLow || bin > binHigh) {
      ...
      if (Kind == "Inclusive"){
	if (j==0){
	  TLatex* txtInfoA = DrawTLatex(0.185, 0.326, Kind+" yield", 0.04, 62, 1);
	  TLatex* txtInfoB = DrawTLatex(0.185, 0.375, "ALICE #bf{Performance}", 0.05, 62, 1);
	  TLatex* txtInfoC = DrawTLatex(0.185, 0.276, "pp #sqrt{s} = 13.6 TeV", 0.04, 42, 1);
	  TLatex* txtInfoD = DrawTLatex(0.185, 0.223, "Min. Bias", 0.04, 42, 1);
	...
	if (j==0){
	  ...
	  TLegend* legend = new TLegend(0.62,0.54,0.92,0.69); //LEFT x, y, Right x, y
	  legend->SetFillColor(4000);
	  legend->SetBorderSize(0);
	  legend->SetTextSize(0.05);
	  legend->AddEntry(hUSS_Y[j], "K^{+}#pi^{-}", "lpf");
	  ...
	  TLegend* legend = new TLegend(0.52,0.18,0.82,0.33); //LEFT x, y, Right x, y
	  legend->SetFillColor(4000);
	  legend->SetBorderSize(0);
	  legend->SetTextSize(0.05);
	  legend->AddEntry(hUSS_Y[j], "K^{+}#pi^{-}", "lpf");
	...
	if (j==0){
	  ...
	  TLatex* txtInfoB = DrawTLatex(0.657, 0.929, "ALICE #bf{Performance}", 0.05, 62, 1);
	...
	if (j==1||j==2||j==3||j==4){
      ...
      plots->SaveAs(Form("../kstar-in-jets/Plot/"+localpath+"/projection_%1.1f_%1.1f.pdf", ptlow, pthigh),"RECREATE");
      // ========================================================================================
  ...
  if (Kind == "Inclusive"){
	...
	//Tmp1
	//======
	fBgTmp[k] = new TF1(Form("kstar_ExtraBgTmp_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x)+pol4(4)", ExtraMinRange, BgMaxRange);
	...
	//Tmp2
	//======
	fBgTmp2[k] = new TF1(Form("kstar_ExtraBgTmp2_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x)+pol4(4)", ExtraMinRange, BgMaxRange);
	...
	if(k==6){
	  ...
	  for (int bin = hUSS_Y_kstarSigTmp2[k]->FindBin(0.78); bin <= hUSS_Y_kstarSigTmp2[k]->FindBin(0.825); bin++)
	  ...
	  fFit_Tmp2[k]->SetParameter(4, fBgTmp2[k]->GetParameter(4));
	  fFit_Tmp2[k]->SetParameter(5, fBgTmp2[k]->GetParameter(5));
	  fFit_Tmp2[k]->SetParameter(6, fBgTmp2[k]->GetParameter(6));
	  fFit_Tmp2[k]->SetParameter(7, fBgTmp2[k]->GetParameter(7));
	  fFit_Tmp2[k]->SetParameter(8, fBgTmp2[k]->GetParameter(8));
	  ...
	  fFit_Tmp[k]->SetParameter(4, fBgTmp[k]->GetParameter(4));
	  fFit_Tmp[k]->SetParameter(5, fBgTmp[k]->GetParameter(5));
	  fFit_Tmp[k]->SetParameter(6, fBgTmp[k]->GetParameter(6));
	  fFit_Tmp[k]->SetParameter(7, fBgTmp[k]->GetParameter(7));
	  fFit_Tmp[k]->SetParameter(8, fBgTmp[k]->GetParameter(8));
	  ...
	  fFit_Tmp2[k]->SetParameter(2, fBgTmp2_POST[k+1]->GetParameter(2));
	  fFit_Tmp2[k]->SetParameter(3, fBgTmp2_POST[k+1]->GetParameter(3));
	  fFit_Tmp2[k]->SetParameter(4, fBgTmp2_POST[k+1]->GetParameter(4));
	  fFit_Tmp2[k]->SetParameter(5, fBgTmp2_POST[k+1]->GetParameter(5));
	  fFit_Tmp2[k]->SetParameter(6, fBgTmp2_POST[k+1]->GetParameter(6));
	  fFit_Tmp2[k]->SetParameter(7, fBgTmp2_POST[k+1]->GetParameter(7));
	  fFit_Tmp2[k]->SetParameter(8, fBgTmp2_POST[k+1]->GetParameter(8));
	  ...
	  fFit[k]->SetParameter(0, fBgTmp2_POST[k+1]->GetParameter(0));
	  fFit[k]->SetParameter(1, fBgTmp2_POST[k+1]->GetParameter(1));
	  fFit[k]->SetParameter(2, fBgTmp2_POST[k+1]->GetParameter(2));
	  fFit[k]->SetParameter(3, fBgTmp2_POST[k+1]->GetParameter(3));
	  fFit[k]->SetParameter(4, fBgTmp2_POST[k+1]->GetParameter(4));
	  fFit[k]->SetParameter(5, fBgTmp2_POST[k+1]->GetParameter(5));
	  fFit[k]->SetParameter(6, fBgTmp2_POST[k+1]->GetParameter(6));
	  fFit[k]->SetParameter(7, fBgTmp2_POST[k+1]->GetParameter(7));
	  fFit[k]->SetParameter(8, fBgTmp2_POST[k+1]->GetParameter(8));
	  ...
	  for (int bin = hUSS_Y_Bg[k]->FindBin(0.82); bin <= hUSS_Y_Bg[k]->FindBin(0.98); bin++)
	    hUSS_Y_Bg[k]->SetBinError(bin, 0);
	  for (int bin = hUSS_Y_kstarSig[k]->FindBin(0.79); bin <= hUSS_Y_kstarSig[k]->FindBin(0.825); bin++)
	...
	//Tmp2
	//=============================================================
	fBgTmp2_POST[k] = new TF1(Form("kstar_ExtraBgTMP2_POST_%i",k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4)", ExtraMinRange, BgMaxRange);
	fBgTmp2_POST[k]->SetParameter(0, fFit_Tmp2[k]->GetParameter(0));
	fBgTmp2_POST[k]->SetParameter(1, fFit_Tmp2[k]->GetParameter(1));
	fBgTmp2_POST[k]->SetParameter(2, fFit_Tmp2[k]->GetParameter(2));
	fBgTmp2_POST[k]->SetParameter(3, fFit_Tmp2[k]->GetParameter(3));
	fBgTmp2_POST[k]->SetParameter(4, fFit_Tmp2[k]->GetParameter(4));
	fBgTmp2_POST[k]->SetParameter(5, fFit_Tmp2[k]->GetParameter(5));
	fBgTmp2_POST[k]->SetParameter(6, fFit_Tmp2[k]->GetParameter(6));
	fBgTmp2_POST[k]->SetParameter(7, fFit_Tmp2[k]->GetParameter(7));
	fBgTmp2_POST[k]->SetParameter(8, fFit_Tmp2[k]->GetParameter(8));
	...
	legendd->AddEntry(hUSS_Y_Fit[k],"Data", "lep");
	legendd->AddEntry(fBg_POST[k], "Bkg. Fit", "l");
	legendd->AddEntry(fkstar_POST[k], "BreitWignalRelativistic. Fit", "l");
	...
	double margin = 0.3;
	double adjustedMin = yMin * (1 - margin);
	double adjustedMax = yMax * (1 + margin);

	if (adjustedMin <= 0) {adjustedMin = yMin * 0.5;}

	hdummy->SetMinimum(adjustedMin);
	hdummy->SetMaximum(adjustedMax);
	...
	hYield->Draw("SAME");
	numbin->SetLogy();
	numbin->SaveAs("../kstar-in-jets/Plot/"+localpath+"/h_Yield.pdf","RECREATE");
      }
  }
  // Jet
  else if (Kind == "Inside"){
	...
	if (k==6){
	  for (int bin = hUSS_Y_Bg[k]->FindBin(0.84); bin <= hUSS_Y_Bg[k]->FindBin(0.98); bin++)
	    hUSS_Y_Bg[k]->SetBinError(bin, 0);
	  ...
	  for (int bin = hUSS_Y_Bg[k]->FindBin(0.84); bin <= hUSS_Y_Bg[k]->FindBin(1.04); bin++)
	    hUSS_Y_Bg[k]->SetBinError(bin, 0);
	...
	// Draw
	TCanvas* lines = new TCanvas(Form("line_%d",k), "", 940, 800);
	lines->SetLeftMargin(0.16);
	lines->SetRightMargin(0.06);
	lines->SetTopMargin(0.08);
	lines->SetBottomMargin(0.14);

	HistoAxisTitles(hUSS_Y_Fit[k], "#it{M}_{ inv}^{ K^{+}#pi^{-} }[GeV/#it{c}^{2}]",
			"d#it{N}^{ K^{+}#pi^{-}}_{USS-LSS}/d#it{M}_{ inv}", 0.06, 62, 1.0, 0.04, 62, 1.6);
	hUSS_Y_Fit[k]->SetTitle(Form(""));
	hUSS_Y_Fit[k]->SetLineColor(1);
	if(k==0){
	  hUSS_Y_Fit[k]->GetYaxis()->SetRangeUser(0.0001, hUSS_Y_Fit[k]->GetMaximum()*1.2);
	}
	...
	TLegend *legendd = new TLegend(0.55,0.48,0.66,0.58);
	legendd->SetFillColor(4000);
	legendd->SetBorderSize(0);
	legendd->SetTextSize(0.03);
	legendd->AddEntry(hUSS_Y_Fit[k],"Data", "lep");
    ...
    for (int k=0; k<2; k++) {
      ...
      if (k==0){
	for (int bin = hUSS_Y_Bg[k]->FindBin(0.80); bin <= hUSS_Y_Bg[k]->FindBin(1.05); bin++)
	  hUSS_Y_Bg[k]->SetBinError(bin, 0);
	for (int bin = hUSS_Y_kstarSig[k]->FindBin(0.75); bin <= hUSS_Y_kstarSig[k]->FindBin(0.82); bin++)
	  hUSS_Y_kstarSig[k]->SetBinError(bin, 0);
