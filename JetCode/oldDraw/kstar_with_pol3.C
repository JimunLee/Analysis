...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (int k=1; k<nPtBins+1; k++)
      ...
      else
	{
      ...
      // fitting kstar
      //kstar
      fBW_kstar[nPtBins-k] = new TF1(Form("kstar_bw_%i",nPtBins-k), "[0]*BreitWignerRelativistic(x, [1], [2])", kstarMinRange, kstarMaxRange);
      ...
      // ===============
      // fitting entire 
      fFit[nPtBins-k] = new TF1(Form("Fullfit_%i", nPtBins-k), "[0]*BreitWignerRelativistic(x, [1], [2]) + pol3(6) + [3]*BreitWignerRelativistic(x, [4], [5]) +[10]*BreitWignerRelativistic(x, [11], [12])", Bglow, Bghigh);
      ...
      fFit[nPtBins-k]->FixParameter(4, fBW_rho[nPtBins-k]->GetParameter(1));
      ...
      fFit[nPtBins-k]->FixParameter(11, fBW_omega[nPtBins-k]->GetParameter(1));
