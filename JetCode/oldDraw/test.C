...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  if (Kind == "Inclusive"){
	...
	hYield->GetXaxis()->SetTitle("p_{T} [GeV/#it{c}]");
	hYield->GetYaxis()->SetTitle("Counts");
	hYield->SetTitle("#frac{d^{2}N_{#K^*}}{dp_{T}d#eta}");
	TH1D* hdummy = new TH1D("hdummy", "hdummy", 1, 0,10);
	hdummy->Draw();
	hYield->Draw("SAME");
  ...
  // Jet
	...
	if (k>5){
	  for (int bin = hUSS_Y_Bg[k]->FindBin(0.86); bin <= hUSS_Y_Bg[k]->FindBin(0.92); bin++)
	    hUSS_Y_Bg[k]->SetBinError(bin, 0);
	}
	else{
	...
	if (k<=5){
	  ...
	  for (int bin = hUSS_Y_kstarSig[k]->FindBin(0.77); bin <= hUSS_Y_kstarSig[k]->FindBin(0.85); bin++)
	    hUSS_Y_kstarSig[k]->SetBinError(bin, 0);
	}
	ptlow = xBins[k];
	pthigh = xBins[k+1];
    ...
    for (int k=0; k<nPtBins; k++)
      {
	TCanvas* redrawCanvas = new TCanvas(Form("redraw_%d", k), "", 940, 800);
	redrawCanvas->SetLeftMargin(0.12);
	redrawCanvas->SetRightMargin(0.1);
	redrawCanvas->SetTopMargin(0.08);
	redrawCanvas->SetBottomMargin(0.14);
	...
	// 히스토그램 축 제목 설정
	HistoAxisTitles(hUSS_Y_Fit[k], 
			"#it{M}_{ inv}^{ K^{+}#pi^{-} }[GeV/#it{c}^{2}]", 
			"1/#it{N}_{ ev} d#it{N}^{ K^{+}#pi^{-}}_{USS-LSS}/d#it{M}_{ inv}", 
			0.06, 62, 1.0, 0.04, 62, 1.0);

	hUSS_Y_Fit[k]->SetTitle("");
	hUSS_Y_Fit[k]->SetLineColor(1);
	hUSS_Y_Fit[k]->Draw();
	...
	// 저장된 fBg_POST와 fkstar_POST도 그리기
	fBg_POST[k]->SetLineColor(4);
	fBg_POST[k]->SetLineWidth(4);
	fBg_POST[k]->Draw("SAME");
	...
	TLatex* txtInfoBB = DrawTLatex(0.56, 0.88, Kind + " yield", 0.04, 62, 1);
	TLatex* txtInfoCC = DrawTLatex(0.56, 0.84, "pp #sqrt{s} = 13.6 TeV", 0.04, 42, 1);
	TLatex* txtInfoDD = DrawTLatex(0.56, 0.74, 
				       Form("%.1f < #it{p}_{T}^{ K*} < %.1f GeV/#it{c}", xBins[k], xBins[k+1]), 
				       0.04, 42, 1);

	// 플롯 저장
	redrawCanvas->SaveAs(Form("../kstar-in-jets/Plot/" + localpath + "/redraw_fit_%1.1f_%1.1f.png", xBins[k], xBins[k+1]));
    
	double HistY = 0;
	double HistYerr = 0;
	double lowM = hUSS_Y_Fit[k]->FindBin(0.85);
	double highM = hUSS_Y_Fit[k]->FindBin(0.94);
	HistY=hUSS_Y_Fit[k]->IntegralAndError(lowM, highM, HistYerr);
	double binW = hUSS_Y_Fit[k]->GetBinWidth(50); //Any number
	double BgY = fBg_POST[k]->Integral(0.85,0.94)/binW;
	...
	covTot = ptr->GetCovarianceMatrix();
	covGG_err = covTot.GetSub(0,8,0,8);// these have to be fBG parameters,
	Double_t Bg_err = (1/binW)*(fBg_POST[k]->IntegralError(0.85,0.94,fBg_POST[k]->GetParameters(),covGG_err.GetMatrixArray()));
