...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (Int_t i=0; i<nPtBins; i++)
    {
      ...
      hUSS_Y_ExtraBg[i] = new TH1D(Form("hUSS_Y_Extra_%i", i), Form("hUSS_Y_Extra_%i", i), nBins, imlow, imtop);
  ...
  for (int k=1; k<nPtBins+1; k++)
	...
	continue;
      ...
      for (int bin = hUSS_Y_Bg[k]->FindBin(0.825); bin <= hUSS_Y_Bg[k]->FindBin(1.15); bin++)
	hUSS_Y_Bg[k]->SetBinError(bin, 0);

      for (int bin = hUSS_Y_ExtraBg[k]->FindBin(0.825); bin <= hUSS_Y_ExtraBg[k]->FindBin(1.05); bin++)
	...
	hUSS_Y_kstarSig[k]->SetBinError(bin, 0);
      ...
      // 2. Background fit
      fExtraBg[k] = new TF1(Form("Extra_bg_%i",k), "pol3", ExtraMinRange, ExtraMaxRange);
      ...
      // fitting entire
      if(k==1){
	fFit[k] = new TF1(Form("Fullfit_%i", k), "exp([0] + [1]*x + [2]*x*x) + pol3(3) + [7]*BreitWignerRelativistic(x, [8], [9])", ExtraMinRange, BgMaxRange);
	...
	fFit[k]->SetParameter(9, fkstar[k]->GetParameter(2));
      ...
      // First
      fBg_POST[k] = new TF1(Form("kstar_ExtraBg_POST_%i",k), "exp([0] + [1]*x + [2]*x*x) + pol3(3)", ExtraMinRange, BgMaxRange);
