...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (int k=1; k<nPtBins+1; k++)
	...
	continue;
      ...
      for (int bin = hUSS_Y_Bg[k]->FindBin(0.82); bin <= hUSS_Y_Bg[k]->FindBin(0.97); bin++)
	hUSS_Y_Bg[k]->SetBinError(bin, 0);

      for (int bin = hUSS_Y_Fit[k]->FindBin(0.82); bin <= hUSS_Y_Fit[k]->FindBin(0.97); bin++)
	...
	fFit[k] = new TF1(Form("Fullfit_%i", k), "exp([0] + [1]*x + [2]*x*x + [3]*x*x*x) + pol4(4)", ExtraMinRange, BgMaxRange);
	...
	fFit[k]->SetParameter(8, fExtraBg[k]->GetParameter(4));
      ...
      // Second
      fExtraBg_POST[k] = new TF1(Form("kstar_kstarBg_POST_%i",k), "pol4", ExtraMinRange, BgMaxRange);
      
      fExtraBg_POST[k]->SetParameter(0, fFit[k]->GetParameter(4));
      fExtraBg_POST[k]->SetParameter(1, fFit[k]->GetParameter(5));
      fExtraBg_POST[k]->SetParameter(2, fFit[k]->GetParameter(6));
      fExtraBg_POST[k]->SetParameter(3, fFit[k]->GetParameter(7));
      fExtraBg_POST[k]->SetParameter(4, fFit[k]->GetParameter(8));
