  ...
  for (int k=1; k<nPtBins+1; k++)
    {
      ...
      //rho
      fBW_rho[nPtBins-k] = new TF1(Form("rho_bw_%i", nPtBins-k), "[6]*BreitWigner(x, [7], [8])", rhoMinRange, rhoMaxRange);
      ...
      // ===============
      // fitting entire 
      fFit[nPtBins-k] = new TF1(Form("Fullfit_%i", nPtBins-k), "[0]*BreitWigner(x, [1], [2]) + [3]*BreitWignerRelativistic(x, [4], [5]) + [6]*BreitWigner(x, [7], [8])", Bglow, Bghigh);
