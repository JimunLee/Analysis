...
void DrawBW(TString Kind, TString USS, TString LSS)
{
  ...
  for (int k=1; k<nPtBins+1; k++)
	...
	hUSS_Y_Bg[k]->SetBinError(bin, 0);
      ...
      for (int bin = hUSS_Y_Fit[k]->FindBin(0.825); bin <= hUSS_Y_Fit[k]->FindBin(1.05); bin++)
	hUSS_Y_Fit[k]->SetBinError(bin, 0);
      ...
      // 1. Extra fit
      fBg[k] = new TF1(Form("Bg_%i",k), "exp([0] + [1]*x + [2]*x*x)", ExtraMinRange, BgMaxRange);
      hUSS_Y_Bg[k]->Fit(fBg[k], "0", "", ExtraMinRange, BgMaxRange);
      
      // 2. Background fit
      fExtraBg[k] = new TF1(Form("Extra_bg_%i",k), "pol3", ExtraMinRange, BgMaxRange);
	...
	/* fFit[k]->SetParameter(9, fkstar[k]->GetParameter(2)); */
      ...
      /* fExtraBg[k]->SetLineColor(2); */
      /* fExtraBg[k]->SetLineWidth(4); */
      /* fExtraBg[k]->Draw("SAME"); */

      /* fBg_POST[k]->SetLineColor(7); */
